...
export const ValidateIncome = {
  common: {
    ...
    // Tên doanh nghiệp
    businessName(value: string, position?: string){
      return { valid: !!value, field: 'businessName', role: 'empty', position: position }; 
    },


    // Mã số thuế
    tax(value: string | null, position?: string, isValid?: boolean){
      if (isValid){
    ...
    phone(value: string | null, position?: string, isValid?: boolean){
      ...
      if (!~PhoneMobileVN.indexOf(value.slice(0, 3))){
  ...
  salary(data: ILOANNormalStorageIncomeSalaryActive){
    ...
    data?.data?.forEach(
      (d) => {
        ...
        const vCompanyCate = ValidateIncome.common.companyCate(d.companyCate, d.uuid);
        if (!vCompanyCate.valid) return isValid = vCompanyCate;

        const vTax = ValidateIncome.common.tax(d.tax, d.uuid);
        ...
        const vPhone = ValidateIncome.common.phone(d.phone, d.uuid);
  ...
  company(data: ILOANNormalStorageIncomeCompanyActive){
    ...
    data?.data?.forEach(
      (d, index) => {
        ...
        const vName = ValidateIncome.common.businessName(d.name, d.uuid);
        if (!vName.valid) return isValid = vName;
  ...
  balance(data: ILOANNormalStorageIncomeBalance){
    ...
    // const vTotalIncome = ValidateIncome.common.totalIncome(null);
    // if (!vTotalIncome.valid) return vTotalIncome;

    const vTotalCost = ValidateIncome.common.totalCost(data.totalCost);
    if (!vTotalCost.valid) return vTotalCost;
