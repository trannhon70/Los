...
import { ILOANNormalStorageIncomeAssetRent, ILOANNormalStorageIncomeDeclare, ILOANNormalStorageIncomeState, Document } from "types/models/loan/normal/storage/Income";
...
export const saveINCOMESalary = ( 
  ...
  const bodySalary = {
    ...
    salaries:dataPersonUUID?.salary.data.map((sal,idx)=>({
        ...
        company_info:{
          business_field:{
            id:sal.career,
            name: careerMetadata?.find(item => item.id === sal.career)?.name ?? '',
          }, 
        ...
        documents: sal.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            ...
            data_file:dos.data_file?.map(dataF =>({
              ...
              created_by_name: dataF.created_by_name,
              updated_by_name: dataF.updated_by_name,
              uuid: dataF.uuid
            }))
          }
        }
  ...
  if(bodySalary.salaries?.length === 0){
    return apiDelete<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-salary/:person_uuid", los_uuid, person_uuid)); // change api step 2
  }
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-salary/:person_uuid", los_uuid, person_uuid), bodySalary);// change api step 2
};
...
export const saveINCOMEAssrentRent = (
        ...
        return (
          {
            ...
            real_estates: ren.assetDetailRealEstate.data?.map((real,idx)=>{
              ...
              documents: real.documents?.length > 0 ? real.documents?.map((dos,idxDocs)=>{
                if(dos.data_file.length > 0){
                  return{
                  document_id: dos.document_id,
                  document_name: dos.document_name,
              ...
              return dos
              }) : generateEmptyDocument(21, config)
            })}),
          } 
        )
      }
      case 'TRANSPORT':{
        return ({
          ...
          asset_transportations:ren.assetDetailTransport.data.map((tran,idx)=>({
            ...
            documents: tran.documents?.length > 0 ? tran.documents?.map((dos,idxDocs)=>{
              if(dos.data_file.length > 0){
                return{
                document_id: dos.document_id,
                document_name: dos.document_name,
      ...
      case 'OTHER':{
        return ({ 
          ...
          other_assets: ren.assetDetailOther.data.map((othe,idx)=>({
            ...
            documents: othe.documents?.length > 0 ? othe.documents?.map((dos,idxDocs)=>{
              if(dos.data_file.length > 0){
                return{
                document_id: dos.document_id,
                document_name: dos.document_name,
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-asset-rent/:person_uuid", los_uuid,person_uuid), bodyAssrent);
};
...
export const saveINCOMEBusiness = (
  ...
  const bodyBusiness = {
    ...
    business_households: dataPersonUUID?.business?.data?.map((item, idx)=>({
        ...
        documents:item.documents.length > 0 ? item.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            return{
            document_id: dos.document_id,
            document_name: dos.document_name,
        ...
        return dos
        }) : generateEmptyDocument(22, config)
      }))
    }
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-business-household/:person_uuid", los_uuid,person_uuid), bodyBusiness);
};
...
export const saveINCOMECompany = (
  ...
  const bodyCompany = {
      ...
      companies: dataPersonUUID?.company.data.map((co,idx)=>({
        ...
        documents: co.documents.length > 0 ? co.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            return{
            document_id: dos.document_id,
            document_name: dos.document_name,
        ...
        return dos
        }) : generateEmptyDocument(23, config)
      }))
  }
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-company/:person_uuid", los_uuid,person_uuid), bodyCompany);
};
...
export const saveINCOMEStock = (
  ...
  const bodyStock = {
      ...
      source_income_stocks: dataPersonUUID?.stock.data.map((sto,idx)=>({
        ...
        documents: sto.documents?.length > 0 ? sto.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            return{
            document_id: dos.document_id,
            document_name: dos.document_name,
        ...
        return dos
        }) : generateEmptyDocument(24, config)
      }))
  }
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-stock/:person_uuid", los_uuid,person_uuid), bodyStock);
};
...
export const saveINCOMEPension = (
  ...
  const bodyPension = {
      ...
      documents: dataPersonUUID?.pension?.documents && dataPersonUUID?.pension?.documents?.length > 0 ? dataPersonUUID?.pension?.documents?.map((dos,idxDocs)=>{
        if(dos.data_file.length > 0){
          return{
            document_id: dos.document_id,
            document_name: dos.document_name,
      ...
      return dos
      }) : generateEmptyDocument(26, config)
  }

  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-pension/:person_uuid", los_uuid,person_uuid), bodyPension);
};
...
export const saveINCOMEDeposit = (
  ...
  const bodyDeposit= {
    ...
    source_income_deposits: dataPersonUUID?.deposit?.data.map((des, idx) => ({
        ...
        documents: des.documents?.length > 0 ? des.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            return{
            document_id: dos.document_id,
            document_name: dos.document_name,
        ...
        return dos
        }) : generateEmptyDocument(25, config)
    }))
  }
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-deposit/:person_uuid", los_uuid,person_uuid), bodyDeposit);
};
...
export const saveINCOMEOther = (
  ...
  const bodyOther = {
    ...
    income_other: dataPersonUUID?.other.data.map((oth, idx)=>({
        ...
        documents: oth.documents?.length > 0 ? oth.documents?.map((dos,idxDocs)=>{
          if(dos.data_file.length > 0){
            return{
            document_id: dos.document_id,
            document_name: dos.document_name,
        ...
        return dos
        }) : generateEmptyDocument(27, config)
      })
    )
  }
  ...
  return apiPost<unknown>(formatPath("v2/normal-loan/:los_uuid/source-income/income-other/:person_uuid", los_uuid,person_uuid), bodyOther);
};
  ...
  const bodyBalance = {
    ...
    cost_detail: {
      interest_payment_expenses: {
        id: "COST_LOAN",
        name: "VNĐ",
        value: incomeStorage?.balance?.interestPayment.value ?? 0,
        desc: incomeStorage?.balance?.interestPayment.description ?? null,
      },
      family_cost: {
        id: "COST_LIVING",
        name: "VNĐ",
        value: incomeStorage?.balance?.familyCost.value ?? 0,
        desc:  incomeStorage?.balance?.familyCost.description ?? null,
      },
      other_cost: {
        id: "COST_OTHER",
        name: "VNĐ",
        value: incomeStorage?.balance?.otherCost.value ?? 0,
        desc:  incomeStorage?.balance?.otherCost.description ?? null

      },
      disb_cost: {
        id: "COST_DISB",
        name: "VNĐ",
        value: incomeStorage?.balance?.disbCost.value ?? 0,
        desc:  incomeStorage?.balance.disbCost.description ?? null
    }
    }
  }
