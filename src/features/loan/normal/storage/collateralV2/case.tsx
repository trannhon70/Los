import { Draft, PayloadAction } from "@reduxjs/toolkit";
import * as _ from 'lodash';
import { ILOANNormalState } from "types/models/loan/normal";
import { ILOANNormalDocument, ILOANNormalFile, IResDocument } from "types/models/loan/normal/configs/Document";
import {
  ICertificateLegalInfoData, ICertificateLegalInfoDataUserList, ICertificateUsePurposes, ICollateralFormGet, ICountCarousel, ICTXDLandData, IDepartment, IDepartmentInfo,
  IDepartmentInfoLand, IInfoAuthorize, ILandAssetType, ILandLegalInformationOwner, ILandOwner, ILandWrapper,
  ILandWrapperCheck, ILegalLandInformatioAsset,
  ILOANNormalCollateralData, ILOANNormalCollateralV2State, ILOANNormalStorageLoanValidate, ILOANNormalStorageV2CollateralForm, IMaketCertificates, IMaketInfo,
  IPerson,
  IPersonCertificateLegal, ISubItems, ISubtype, ISubTypeGet, ITypeActionSaveDocumentCollateral,
  ITypeActionSaveDocumentCollateralMetaData, ITypeCTXD, ITypeMappingDocumentAction, IValueOnChangeCollateral
} from "types/models/loan/normal/storage/CollaretalV2";
import { ILOANNormalStorageAddress } from "types/models/loan/normal/storage/Legal";
import { generateUUID, PREFIX_LOCAL } from "utils";
import { CheckboxValue } from "views/components/base/Checkbox";
import { generaterNameCollateral } from "views/pages/LOAN/utils";
import { IDeleteParams } from "./api";
import { autoFillBDS, autofillDEPARTTTTTTTT, autofillLANDDDDDD, autofillMARKETTTT, autofillRestMarket } from "./autofill";
import {
  generateEmptyData,
  generateEmptyDatacertificateUsePurposes,
  generateEmptyDataCollateralCertificateUseListLegal,
  generateEmptyDataCollateralRPROSubType,
  generateEmptyDataCollateralSubType,
  generateEmptyDataCollateralSubTypeCTXD,
  generateEmptyDataCollateralSubTypeLandCTXD,
  generateEmptyDataCtxdGcnQshData,
  generateEmptyDataDepartmentCertificates,
  generateEmptyDataDepartmentInfo,
  generateEmptyDataMaketCertificates,
  generateEmptyTypeTransportLegal,
  generateEmptyTypeTransportLegalDocumentsChild
} from "./emptyData";
export enum ETypeCollateral {
  ALL = "ALL",
  REST = "REST", // Bất động sản
  MEST = "MEST", // PTVT/Động sản
  DEVI = "DEVI", // MMTB; Dây truyền sản xuất
  GODS = "GODS", // Vật tư hàng hóa
  RPRO = "RPRO", // Quyền tài sản
  STOC = "STOC", // Chứng khoán
  BALC = "BALC", // Số dư TKTG, HTTG, GTCG
  OTHE = "OTHE", // Tài sản khác
}


export enum ESubtypeRest {
  LAND = "LAND", // Đất / nhà riêng lẻ
  APPA = "APPA", // Căn hộ chung cư
  MARK = "MARK", // Sạp chợ
}

export enum ETypeCollateralChildRestLand {
  LANS = "LANS", //QSDĐ là đất và / hoặc Nhà riêng lẻ
  INDU = "INDU", // QSDĐ là đất thuê KCN
  HIRI = "HIRI", // Nhà nhiều hộ, nhiều tầng
  REIN = "REIN", // QSDĐ là đất thuê ngoài KCN
  BRIN = "BRIN", // QSDĐ là đất có nguồn gốc vừa giao vừa thuê ngoài KCN
  OTHL = "OTHL", // Bất động sản khác
  PROJ = "PROJ" // Dự án
}

export enum ETypeCollateralChildRestAppa {
  URRO = "URRO", // Quyền sở hữu căn hộ chung cư
  NARO = "NARO", // Căn hộ chung cư đã hình thành nhưng chưa được cấp GCN
  FURO = "FURO" // Căn hộ hình thành trong tương lai
}

export enum ETypeCollateralChildRestMark {
  MRMA = "MRMA", // Sạp chợ/ Ô TTM
}

export enum ECheckType {
  CTXD_LAND = "CTXD_LAND",
  CTXD_GCN = "CTXD_GCN",
}

export enum ETypeLand {
  LAND = 0,
  CTXD_LAND = 1,
  CTXD_GCN = 2,
}

export enum ETypeLandName {
  LAND = "LAND",
  CTXD_LAND = "CTXD_LAND",
  CTXD_GCN = "CTXD_GCN",
}

export enum EActionMenu {
  DELETE = "delete",
}

export enum EDetailsTransport {
  TRVE = "TRVE",
  SPVE = "SPVE",
  NRVE = "NRVE"
}

export const CollateralCaseV2 = {

  

  addNewItemTypeCollatetal(
    state: Draft<ILOANNormalState>,
    action: PayloadAction<string>
  ) {
    const uuidActiveData = generateUUID();
    const uuidActiveSubtype = generateUUID();
    const uuidActiveSubtypeItem = generateUUID()
    state.storage.collateral_v2.uuidActiveData = uuidActiveData;
    state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(col => ({
      ...col,
      is_collapse_type : false
    }))
    
    state.storage.collateral_v2.data.push({
      ...generateEmptyData(),
      uuidActiveData: uuidActiveData,
      uuidActiveSubtype: uuidActiveSubtype,
      documents: [],
      sub_type: [
        {
          uuidActiveSubtype: uuidActiveSubtype,
          is_collapse_sub_type: false,
          id: "",
          child_sub_type: "",
          items: [{ ...generateEmptyDataCollateralSubType(), activeUUID: uuidActiveSubtypeItem }],
          uuidItemsActive: uuidActiveSubtypeItem,
        },
      ],
    });
  },

  setCollapseType(
    state: Draft<ILOANNormalState>,
    action: PayloadAction<number | undefined>
  ) {
    state.storage.collateral_v2.data = state.storage.collateral_v2.data.map((d, index) => {
      if (index === action.payload) {
        return {
          ...d,
          is_collapse_type: !d.is_collapse_type
        }
      } else {
        return {
          ...d,
          is_collapse_type: false
        }
      }
    });
  },

  setCollapseSubType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidSubTypeActive: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.payload) {
          d.is_collapse_type &&
            d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubTypeActive) {
                st.is_collapse_sub_type = !st.is_collapse_sub_type;
              } else {
                st.is_collapse_sub_type = false;
              }
              return { ...st };
            });
        }
        return { ...d };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidSubTypeActive: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addNewItemTypeCollatetalSubType(
    state: Draft<ILOANNormalState>,
    action: PayloadAction<string>
  ) {
    const uuidActive = generateUUID();
    state.storage.collateral_v2.data.map((co) => {
      if (co.uuidActiveData === action.payload) {
        co.uuidActiveSubtype = uuidActive;
        co.sub_type.push({
          ...generateEmptyDataCollateralRPROSubType(),
          id: action.payload,
          uuidActiveSubtype: uuidActive,
        });
      }
      return { ...co };
    });
  },

  /**
   * @todo Xóa subType
   * @param payload uuid subType
   * @param meta uuid data
   *
   */
  deleleteSubItem: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map(i => {
        if (i.is_collapse_type === true) {
          i.sub_type = i.sub_type.map(sub => {
            return { ...sub, id: '', child_sub_type: "", items: [], uuidItemsActive: "", is_collapse_sub_type: false }
          })
        }
        return { ...i }
      })
    },
    prepare(payload: string, meta: { uuidData: string }) {
      return { payload, meta };
    },
  },

  /**
   * action payload uuidItem
   *
   */
  deleleteItem: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type.map((st,stIndex) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              st.items = st.items.filter(
                (i) => i.activeUUID !== action.payload
              );
              st.uuidItemsActive = st.items[0]?.activeUUID ?? ""
            }
            return { ...st };
          });
        }
        return { ...d };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },

  deleleteSubtypeItem: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type.map((st) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              st.items = st.items.filter(
                (i) => i.activeUUID !== action.payload
              );
              st.uuidItemsActive = st.items[0]?.activeUUID ?? ''
            }
            return { ...st };
          });
        }
        return { ...d };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },

  setTypeCollateral: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<string, string, { uuidData: string }>
    ) {
      let typeCollateral = action.payload;

      // let isValidType = state.storage.collateral_v2.data.find(
      //   (d) => d.type === typeCollateral
      // );

      // /**
      //  *  TODO: Nếu type đã tồn tại
      //  *  Sẻ add thêm vào Subtype của đối tượng đó
      //  *  Xóa đối tượng type trùng đi
      //  *
      //  */
      // if (isValidType) {
      //   const uuidActiveSubtype = generateUUID();
      //   state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
      //     (d) => {
      //       if (d.type === typeCollateral) {
      //         d.sub_type.push({
      //           uuidActiveSubtype: uuidActiveSubtype,
      //           is_collapse_sub_type: false,
      //           id:
      //             isValidType?.type === ETypeCollateral.REST
      //               ? ""
      //               : isValidType?.type ?? "",
      //           items: [],
      //           uuidItemsActive: "",
      //         });
      //         d.is_collapse_type = true;
      //       }
      //       return { ...d };
      //     }
      //   );

      //   state.storage.collateral_v2.data = state.storage.collateral_v2.data.filter(
      //     (d) => d.uuidActiveData !== action.meta.uuidData
      //   );
      // } else {



      /**
       *  TODO: Trường hợp không trùng
       *  Sẻ khởi tạo bình thường
       *
       *  Thêm dữ liệu Item
       *  Set lại is_collapse_type
       *  Chỉ một is_collapse_type = true
       */
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.type = action.payload;
          d.sub_type[0].id =
            (d.type === ETypeCollateral.REST || d.type === ETypeCollateral.MEST ||  d.type === ETypeCollateral.DEVI) ? "" : action.payload; //
          d.is_collapse_type = true;
        } else {
          d.is_collapse_type = false;
        }
        return { ...d };
      });

      // TODO: check type có tồn tại không
      let isValidCarousel = state.storage.collateral_v2.carousel?.find(
        (c) => c.type === typeCollateral
      );

      // TODO: Cập nhật số lượng tài sản
      if (isValidCarousel) {
        state.storage.collateral_v2.carousel.map((c) => {
          if (c.type === typeCollateral) {
            c.total = c.total + 1;
          }
          return { ...c };
        });
      } else {
        state.storage.collateral_v2.carousel.push({
          name: generaterNameCollateral(typeCollateral),
          type: typeCollateral,
          total: 1,
        });
      }
    },
    prepare(payload: string, meta: { uuidData: string }) {
      return { payload, meta };
    },
  },

  setTypeLand: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        number,
        string,
        { uuidData: string; uuIdSubType: string }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type.map((st) => {
            const uuIdItemACtive = st?.uuidItemsActive ?? "";
            if (st.uuidActiveSubtype === action.meta.uuIdSubType) {

              st.items.map((i) => {
                if (i.activeUUID === uuIdItemACtive) {
                  if (action.payload === 0) {
                    return (i.type_land = ETypeLandName.LAND);
                  } else if (action.payload === 1) {
                    return (i.type_land = ETypeLandName.CTXD_LAND);
                  } else if (action.payload === 2) {
                    return (i.type_land = ETypeLandName.CTXD_GCN);
                  } else {
                    return (i.type_land = "");
                  }
                }
                return { ...i };
              });
            }
            return { ...st };
          });
        }
        return { ...d };
      });
    },
    prepare(payload: number, meta: { uuidData: string; uuIdSubType: string }) {
      return { payload, meta };
    },
  },

  // TODO: aciton => collateral_type
  setCollateralTypeActive(
    state: Draft<ILOANNormalState>,
    action: PayloadAction<string>
  ) {
    state.storage.collateral_v2.activeType = action.payload;
  },

  // TODO: Xóa tắt cả tài sản, xoa type khỏi carousel
  // aciton => uuId Collateral
  deleteAllCollateral: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<IDeleteParams, string ,{key: string}>
    ) {
      if(action.meta.key === "item"){
        state.storage.collateral_v2.data.map((d) => {
          if (d.uuidActiveData === action.payload.uuid) {
            d.price_cert_uuid = "";
            d.isSaved =false;
            d.collateral_value=0;
            d.sub_type[0].items = []
          }
          return {...d}
        })
      }else {
        let collateral_type = state.storage.collateral_v2.data?.find(
          (d) => d.price_cert_uuid === action.payload.cert_uuid
        )?.type;
        const count = state.storage.collateral_v2.carousel.find((c) => c.type === collateral_type)?.total ?? 0;
        if (action.payload.cert_uuid) {
          if (count > 1) {
            state.storage.collateral_v2.carousel = state.storage.collateral_v2.carousel.map(
              c => {
                if (c.type === collateral_type) {
                  c.total = c.total - 1
                }
                return { ...c }
              }
            );
          } else {
            state.storage.collateral_v2.carousel = state.storage.collateral_v2.carousel.filter(
              (c) => c.type !== collateral_type
            );
          }
  
          state.storage.collateral_v2.data = state.storage.collateral_v2.data.filter(
            (d) => d.price_cert_uuid !== action.payload.uuid
          );
          state.storage.collateral_v2.data = state.storage.collateral_v2.data.filter(
            (d) => d.uuidActiveData !== action.payload.uuid
          );
        } else {
  
          let _collateralTypeUuid = state.storage.collateral_v2.data?.find(d => d.uuidActiveData === action.payload.uuid)?.type;
          state.storage.collateral_v2.carousel = state.storage.collateral_v2.carousel?.map(
            c => {
              if (c.type === _collateralTypeUuid) {
                c.total = c.total - 1
              }
              return { ...c }
            }
          );
          state.storage.collateral_v2.data = state.storage.collateral_v2.data.filter(
            (d) => d.uuidActiveData !== action.payload.uuid
          );
        }
  
        state.storage.collateral_v2.data = state.storage.collateral_v2.data.filter(
          (d) => d.uuidActiveData !== action.payload.uuid
        );
      }
     
    },
    prepare(
      payload: IDeleteParams,
      meta: { key:string }
    ) {
      return { payload, meta };
    },
  },



  // TODO: cap nhat gia tri collateral
  onChangeCollaretalProperty: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        IValueOnChangeCollateral,
        string,
        { key: keyof ILOANNormalCollateralData; uuid: string }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuid) {
            return {
              ...d,
              [action.meta.key]: action.payload
            }
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: IValueOnChangeCollateral,
      meta: { key: keyof ILOANNormalCollateralData; uuid: string }
    ) {
      return { payload, meta };
    },
  },
  postCollaterals(state: ILOANNormalState, action: PayloadAction<{
    type: string, 
    onContinue?: boolean, 
    addCollateral?: boolean,
    addItem?: boolean,
    collapseIndex?: number}>) { },
  
  postFileCollaterals:{
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { 
          listFile : ILOANNormalDocument[],
          type:'CollaretalType' | 'LandCTXD' | 'LandCTXDGCN' | 'No-RealEstate' | 'Certificate' | 'Authorize',
          extraData?:any ,
          isDelete: boolean
        }
      >
    ) {},
    prepare(payload: string, meta: { 
      listFile : ILOANNormalDocument[],
      type:'CollaretalType' | 'LandCTXD' | 'LandCTXDGCN' | 'No-RealEstate' | 'Certificate' | 'Authorize',
      extraData?:any,
      isDelete: boolean
    }) {
      return { payload, meta };
    },
  
  },

  postCollateralsIgnore(state: ILOANNormalState, action: PayloadAction<string>) {
    
  },
  updateIgnore(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2.ignore = action.payload
    if(action.payload === "Y" && state.storage.full.data){
      state.storage.full.data.form.collateral_form.is_collateral = true
    }
    else if(action.payload === "N" && state.storage.full.data){
      state.storage.full.data.form.collateral_form.is_collateral = false

      state.storage.full.data.form.collateral_form.data.collaterals = []
      state.storage.full.data.form.collateral_form.data.dashboard = []

      state.storage.collateral_v2 = {
        activeType: ETypeCollateral.ALL,
        uuidActiveData:'',
        carousel: [],
        data: [],
        validate: {
          valid: true
        },
        fetched :false,
        fetching:false,
        ignore:"N",
        originData: []
      }
    }
  },
  updateCollaterals(state: ILOANNormalState, action: PayloadAction<{
    type:string,
    onContinue?: boolean, 
    addCollateral?: boolean,
    addItem?: boolean,
    collapseIndex?: number}>) { },
  /////////quyen tai san /////////
  
  addCollaretalRPRO: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string
      >
    ) {
      // const uuidActive = state.storage.collateral_v2.data.find
      const uuidActiveData = state.storage.collateral_v2.uuidActiveData

      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(data => {
        if(data.uuidActiveData === uuidActiveData){
          const uuidSubtypeActive = data.uuidActiveSubtype;
          return {
            ...data,
            sub_type: data.sub_type.map(subtype => {
              if(subtype.uuidActiveSubtype === uuidSubtypeActive){
                const typeLand = data.type === ETypeCollateral.REST ? ETypeLandName.LAND : "";
                const newItem = {
                  ...generateEmptyDataCollateralSubType(),
                  activeUUID: generateUUID(),
                  type_land: subtype.id === (ESubtypeRest.LAND || ESubtypeRest.APPA || ESubtypeRest.MARK) ? typeLand : "",
                }
                return {
                  ...subtype,
                  uuidItemsActive: newItem.activeUUID,
                  items: [...subtype.items, newItem]
                }
              }
              else return {...subtype}
            })
          }
        }
        else return {...data}
      })
      // state.storage.collateral_v2.data.map((col) => {
      //   if (col.uuidActiveData === action.meta.uuid) {
      //     const isCompactness = col.is_compactness;
      //     col.sub_type.map((i) => {
      //       if (i.uuidActiveSubtype === action.meta.uuidActive) {
      //         const activeUUID = generateUUID();
      //         const typeLand = col.type === ETypeCollateral.REST ? ETypeLandName.LAND : "";
      //         i.uuidItemsActive = activeUUID;
      //         if (isCompactness === "N" && i.items.length === 1) {
      //           return { ...i }
      //         } else {
      //           i.items.push({
      //             ...generateEmptyDataCollateralSubType(),
      //             activeUUID: activeUUID,
      //             type_land: i.id === (ESubtypeRest.LAND || ESubtypeRest.APPA || ESubtypeRest.MARK) ? typeLand : "",
      //           });
      //         }
      //       }
      //       return { ...i };
      //     });
      //   }
      //   return { ...col };
      // });
    },
    prepare(payload: string) {
      return { payload };
    },
  },

  addCollaretalRPRORealStateLand: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuid: string; uuidActive: string }
      >
    ) {
      // const uuidActive = state.storage.collateral_v2.data.find
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuid) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActive) {
              const activeUUID = generateUUID();
              const typeLand =
                col.type === ETypeCollateral.REST ? ETypeLandName.LAND : "";
              i.uuidItemsActive = activeUUID;

              i.items.push({
                ...generateEmptyDataCollateralSubType(),
                activeUUID: activeUUID,
                type_land: i.id === (ESubtypeRest.LAND || ESubtypeRest.APPA || ESubtypeRest.MARK) ? typeLand : "",
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { uuid: string; uuidActive: string }) {
      return { payload, meta };
    },
  },

  setCollaretalRPRO: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        IValueOnChangeCollateral,
        string,
        { uuid: string; uuidActive: string; key: keyof ISubItems }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type = col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                const activeUUIDType = i.uuidItemsActive;
                i.items = i.items?.map((iq) => {
                  if (iq.activeUUID === activeUUIDType) {
                    /**
                     * Map theo loại đất
                     * TH1: CTXD có GCN QSH riêng
                     * TH2: Đất
                     * TH3: CTXD trên đất
                     */
                    if (iq.type_land === ETypeLandName.CTXD_GCN) {
                      let uuidActiveCtxdGcnQsh =
                        iq.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                      iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data = iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data?.map(
                        (cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                            return {
                              ...cgqd,
                              [action.meta.key]: action.payload,
                            };
                          }
                          return { ...cgqd };
                        }
                      );
                    }
                    return {
                      ...iq,
                      [action.meta.key]: action.payload,
                    };
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: IValueOnChangeCollateral,
      meta: { uuid: string; uuidActive: string; key: keyof ISubItems }
    ) {
      return { payload, meta };
    },
  },

  setCollaretalAssessmentInfomation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        IValueOnChangeCollateral,
        string,
        { uuid: string; uuidActive: string; key: keyof (ILandWrapper & ILandWrapperCheck) | 'documents' }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                const activeUUIDType = i.uuidItemsActive;
                i.items = i.items?.map((iq) => {
                  if (i.id === ESubtypeRest.MARK && iq.activeUUID === activeUUIDType) {
                    iq.market.maket_wrapper = {
                      ...iq.market.maket_wrapper,
                      [action.meta.key]: action.payload
                    } ?? {}
                  }
                  else if (i.id === ESubtypeRest.APPA && iq.activeUUID === activeUUIDType) {
                    iq.department.department_wrapper = {
                      ...iq.department.department_wrapper,
                      [action.meta.key]: action.payload
                    }
                  }
                  else if (i.id === ESubtypeRest.LAND && iq.activeUUID === activeUUIDType) {
                    if (iq.activeUUID === activeUUIDType) {
                      /**
                       * Map theo loại đất
                       * TH1: CTXD có GCN QSH riêng
                       * TH2: Đất
                       * TH3: CTXD trên đất
                       */
                      if (iq.type_land === ETypeLandName.LAND) {
                        iq.land.land_wrapper = {
                          ...iq.land.land_wrapper,
                          [action.meta.key]: action.payload
                        }
                      }

                      if (iq.type_land === ETypeLandName.CTXD_LAND) {
                        iq.ctxd_land.ctx_land_wrapper = {
                          ...iq.ctxd_land.ctx_land_wrapper,
                          [action.meta.key]: action.payload
                        }
                      }

                      if (iq.type_land === ETypeLandName.CTXD_GCN) {
                        let uuidActiveCtxdGcnQsh =
                          iq.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data = iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data?.map(
                          (cgqd) => {
                            if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                              if (action.meta.key === 'documents') {
                                cgqd.documents = action.payload as ILOANNormalDocument[];
                              } else {
                                cgqd.ctxd_gcn_qsh_land_info.ctx_land_wrapper = {
                                  ...cgqd.ctxd_gcn_qsh_land_info.ctx_land_wrapper,
                                  [action.meta.key]: action.payload,
                                };
                              }
                            }
                            return { ...cgqd };
                          }
                        );
                      }

                    }
                  }
                  else {
                    return {
                      ...iq,
                      [action.meta.key]: action.payload,
                    };
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: IValueOnChangeCollateral,
      meta: { uuid: string; uuidActive: string; key: keyof (ILandWrapper & ILandWrapperCheck) | 'documents' }
    ) {
      return { payload, meta };
    },
  },

  setCollaretalCurrentValueItem: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        { uuid: string; uuidActive: string, uuidItemActive: string }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                i.items = i.items?.map((iq) => {
                  if (iq?.activeUUID === action.meta.uuidItemActive) {
                    iq.current_value_item = Number(action.payload)
                  }

                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: { uuid: string; uuidActive: string, uuidItemActive: string }
    ) {
      return { payload, meta };
    },
  },
  //
  onChangeCollaretalRPROApproval: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuid: string; uuidActive: string }
      >
    ) {
      state.storageApproval.collateral.data = state.storageApproval.collateral.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                i.uuidItemsActive = action.payload;
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(payload: string, meta: { uuid: string; uuidActive: string }) {
      return { payload, meta };
    },
  },

  onChangeCollaretalRPRO: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuid: string; uuidActive: string }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                i.uuidItemsActive = action.payload;
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(payload: string, meta: { uuid: string; uuidActive: string }) {
      return { payload, meta };
    },
  },

  setSubType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number,
        string,
        {
          uuidSubType: string;
          uuidData: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            // d.sub_type[0].id = action.payload?.toString();
            // d.sub_type[0].is_collapse_sub_type = false;
            d.sub_type?.map((st) => {
              if (
                st.uuidActiveSubtype === action.meta.uuidSubType &&
                action.payload !== null
              ) {
                st.id = action.payload?.toString();
                st.is_collapse_sub_type = false;
              }
              return { ...st };
            });
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: string | number,
      meta: { uuidSubType: string; uuidData: string }
    ) {
      return { payload, meta };
    },
  },

  setChildSubType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidSubType: string;
          uuidData: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            // d.sub_type[0].id = action.payload?.toString();
            // d.sub_type[0].is_collapse_sub_type = false;
            d.sub_type?.map((st) => {
              if (
                st.uuidActiveSubtype === action.meta.uuidSubType &&
                action.payload !== null
              ) {
                st.child_sub_type = action.payload;
              }
              return { ...st };
            });
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: string,
      meta: { uuidSubType: string; uuidData: string }
    ) {
      return { payload, meta };
    },
  },

  setActiveSubtypeItem: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuidCollateral: string; uuidSubtype: string }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidCollateral) {
            col.sub_type = col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubtype) {
                i.uuidItemsActive = action.payload;
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string,
      meta: { uuidCollateral: string; uuidSubtype: string }
    ) {
      return { payload, meta };
    },
  },
  updateAPIStorageCollateralType(   // fetch full list collateral
    state: Draft<ILOANNormalState>,
    action: PayloadAction<ILOANNormalStorageV2CollateralForm>
  ) {
    const payload = action.payload.data.collaterals;
    
    let dataArr = payload?.map(car => {
      return {
        type: car.collateral_type.id,
        name: car.collateral_type.name ?? "",
        total: 1
      }
    })

    //danh sách tổng của các loại
    const counts = {} as ICountCarousel
    dataArr.forEach((x) => {
      counts[x.type] = (Number(counts[x.type]) || 0) + 1;
    });
    // console.log("action.payload",action.payload)

    const dataIgnore = action.payload.is_collateral === true ? "Y" : "N"

    // console.log("dataIgnore",dataIgnore)

    const NewArr = dataArr.map((i) => {
      // kiểm tra cùng loại thì gán tổng của loại đó vào
      if (i.type === Object.keys(counts).find(o => o === i.type)) {
        i = { ...i, total: Number(counts[i.type]) }
      }
      return { ...i }
    })

    // let setObj = new Set(); // create key value pair from array of array

    // let result = NewArr.reduce((acc,item)=>{
    //   if(!setObj.has(item.type)){
    //     setObj.add(item.type,item)
    //     acc.push(item)
    //   }
    //   return acc;
    // },[]);//converting back to array from mapobject

    // filter trùng 
    state.storage.collateral_v2.carousel = [...new Map(NewArr.map(item => [JSON.stringify(item), item])).values()]
    // state.storage.collateral_v2.carousel = result

    // const mappingDocument = (docs: IResDocument[]): ILOANNormalDocument[] => {
    //   if (!docs) return [];
    //   return docs.map(doc => {
    //     const result: ILOANNormalDocument = {
    //       uuid: generateUUID(),
    //       document_id: doc?.document_id,
    //       document_name: doc?.document_name,
    //       child_files: doc?.child_files?.map(resFile => {
    //         return ({ ...resFile, file_upload: '' });
    //       }) ?? [],
    //     }
    //     return result;
    //   })
    // }
     const mappingDocument = (data: IResDocument[]) => {
        if (!data) return [];
        return data.map((doc: IResDocument) => {
          // 
          const result: ILOANNormalDocument = {
            uuid: generateUUID(),
            document_id: doc.document_id,
            document_name: doc.document_name,
            child_files: doc?.child_files?.map((file, index) => {
              const result: ILOANNormalFile = {
                content_type: file.content_type,
                created_at: file.created_at,
                created_by: file.created_by,
                created_by_name: file.created_by_name,
                updated_at: file.updated_at,
                updated_by: file.updated_by,
                updated_by_name: file.updated_by_name,      
                description: file.description,
                display_order: index + 1,
                file_id: index,
                name: file.name,
                uuid: file.uuid,
                custom_keys: file.custom_keys,
                file_upload: '',
              };
              return result;
            }) ?? []
          }
          return result;
        });
      };
    state.storage.collateral_v2.ignore = dataIgnore

    state.storage.collateral_v2.data = payload?.map((item, itemIndex) => {
      const dataActiveUUID = generateUUID();
      const SubtypeActiveUUID = generateUUID();
      state.storage.collateral_v2.uuidActiveData = dataActiveUUID;
      return {
        uuidActiveData: dataActiveUUID, //
        isSaved: true,
        uuidActiveSubtype: SubtypeActiveUUID,
        is_collapse_type: itemIndex === 0 ? true : false,
        type: item.collateral_type?.id,
        status: item?.status?.id,
        is_compactness: item.is_compactness?.id,
        valuation_id: item?.valuation_id,
        valuation_date: (item?.valuation_date) * 1000,
        valuation_unit_type: item.valuation_unit_type?.id,
        valuation_unit: item.valuation_unit?.id,
        valuation_center: item.valuation_center?.id,
        valuation_center_opinion: item.valuation_center_opinion,
        independence_organization: item.independence_organization?.id,
        other_independence_organization: item.other_independence_organization,
        purpose: item.purpose?.id,
        other_purpose: item.other_purpose,
        address: item?.address,
        province: item.province?.id,
        district: item.district?.id,
        ward: item.ward?.id,
        position_type: item.position_type?.id,
        other_position_type: item.other_position_type,
        lane_width: item.lane_width?.id,
        description: item.description,
        collateral_value: item.collateral_value.value,
        max_percentage: item.max_percentage?.value,
        documents: mappingDocument(item?.documents ?? []),
        sub_type: item.sub_types && item.sub_types?.map(i => {
          const activeUUIDItem = generateUUID();

          return {
            is_collapse_sub_type: false,
            uuidItemsActive: activeUUIDItem,
            uuidActiveSubtype: SubtypeActiveUUID,
            id: i.id,
            child_sub_type: i.child_id,
            items: i.items && i.items.map((itemSub, indexItem) => {
              return {
                activeUUID: indexItem === 0 ? activeUUIDItem : generateUUID(),
                type_land: ETypeLandName.LAND,
                departmentActiveUUID: 'departmentActiveUUID',
                maketActiveUuid: 'maketActiveUuid',
                current_value_item: itemSub?.collateral_value?.value,
                ratio: itemSub?.max_percentage?.value,
                value: itemSub?.collateral_value?.value,
                typeCollateral: i.id === "BALC" ? itemSub?.detail.balance_type?.id : itemSub?.detail?.name,
                license: itemSub?.detail?.license_number,
                status: i?.id === 'BALC' || i?.id === 'STOC' || i?.id === 'OTHE' ? itemSub?.detail?.status as string : itemSub?.detail?.status?.id,
                status_flag: itemSub?.detail?.status,
                description: itemSub?.detail?.description,
                collateral_value: itemSub?.collateral_value?.value,
                collateral_code: itemSub?.collateral_code,
                count: itemSub?.detail?.quantity_each_type,
                branch: itemSub?.detail?.brand,
                brand: itemSub?.detail?.brand,
                other_brand: itemSub?.detail?.other_brand,
                model: itemSub?.detail?.model,
                year: Number(itemSub?.detail?.manufacturing_date),
                info_collatetal: itemSub?.description,
                quantity: itemSub?.detail?.quantity,
                number_register: itemSub?.detail?.license_number,
                issuer: itemSub?.detail?.distributor?.id,
                other_issuer: itemSub?.detail?.other_distributor,
                CLCL: itemSub?.detail?.remain_quality?.value,
                production: itemSub?.detail?.origin_of_production,
                origin_of_production: itemSub?.detail?.origin_of_production?.id,
                other_origin_of_production: itemSub?.detail?.other_origin_of_production,
                vehicle_identification_number: itemSub?.detail?.vehicle_identification_number ?? "",
                license_number: itemSub?.detail?.vehicle_identification_number ?? "",
                engine_number: itemSub?.detail?.engine_number ?? "",
                license_plate: itemSub?.detail?.license_plate ?? "",
                departmentInfoActiveUUID: '001001001',
                has_certificate_maket: itemSub?.has_certificate?.id,
                documents:mappingDocument(itemSub?.detail?.documents),
                transportation_sub_type: itemSub?.detail?.transportation_sub_type,
                other_transportation_sub_type: itemSub?.detail?.other_transportation_sub_type,
                legal_document_types: itemSub?.detail?.legal_document_types,
                credit_extension_secured: itemSub?.credit_extension_secured?.id,
                owner_wrapper: {
                  owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                  owner: itemSub?.owner_wrapper?.owners?.map(ow => ({
                    full_name: ow?.full_name,
                    person_uuid: ow?.person_uuid,
                    owner_uuid: ow?.owner_uuid ?? ""
                  })) ?? []
                },
                activeUUIDCertificateUsePurposes: "activeUUIDCertificateUsePurposesdepartment",
                department: {   // type chung cu 
                  department_wrapper: {
                    from_credit_extension: itemSub?.from_credit_extension?.id,
                    is_exploited: itemSub?.is_exploited?.id ?? "N",
                    credit_extension_secured: itemSub?.credit_extension_secured?.id,
                    non_business_area: itemSub?.non_business_area?.value?.toString() ?? null,
                    max_percentage: itemSub?.max_percentage?.value?.toString(),
                    value_of_land: itemSub?.collateral_value?.value, // check ux or API
                    description: itemSub?.description,
                    has_land_asset: itemSub?.has_certificate?.id,// check ux or API
                    has_certificated_land_asset: itemSub?.has_certificate?.id,// check ux or API
                    
                  },
                  department_certificate_legal: itemSub?.certificates?.map((cerr, idx) => ({
                    order: idx + 1,
                    persons: cerr?.persons?.map(per => ({
                      full_name: per?.full_name,
                      person_uuid: per?.person_uuid,
                      apart_owner_cert_item_uuid: per?.apart_owner_cert_item_uuid ?? ""
                    })) ?? [], // check legal
                    uuid_certificate_legal: idx === 0 ? 'departmentActiveUUID' : generateUUID(),
                    other_certificate_type: cerr?.certificate_type?.id,  // check 
                    other_certificate_type_other: cerr?.other_certificate_type, // check
                    certificate_code: cerr?.certificate_code,
                    certificate_no: cerr?.certificate_no,
                    issue_date: cerr?.issue_date ? (cerr?.issue_date ?? 0) * 1000 : null,
                    place_of_issue: cerr?.place_of_issue,
                    contract_type: cerr?.contract_type,
                    contract_number_type: cerr?.contract_type,
                    contract_number: cerr?.contract_no,
                    contract_date: cerr?.contract_date ? (cerr?.contract_date ?? 0) * 1000 : null,
                    documents: mappingDocument(cerr?.documents),
                    apart_owner_cert_uuid: cerr?.apart_owner_cert_uuid ?? ""
                  })),
                  department_info: itemSub?.apartments?.map((depp, ii) => {
                    return {
                      departmentInfoActiveUUID: ii === 0 ? '001001001' : generateUUID(),
                      house_type: depp?.house_type,
                      apartment_type: depp?.apartment_type?.id,
                      other_apartment_type: depp?.other_apartment_type,
                      apartment_number: depp?.apartment_number,
                      block: depp?.block,
                      floor: depp?.floor,
                      start_date: depp?.start_date ? (depp?.start_date ?? 0) * 1000 : null,
                      certificate_area: depp?.certificate_area ?? "",
                      real_area: depp?.real_area?.value,
                      usage_form: depp?.usage_from,
                      duration: depp?.duration,
                      ownership_category: depp?.ownership_category,
                      apart_room_uuid:depp?.apart_room_uuid ?? ""
                    }
                  }),
                  department_info_land: {
                    address: itemSub?.land?.address,
                    province: itemSub?.land?.province?.id,
                    district: itemSub?.land?.district?.id,
                    ward: itemSub?.land?.ward?.id,
                    certificate_address: itemSub?.land?.certificate_address,
                    certificate_province: itemSub?.land?.certificate_province?.id,
                    certificate_district: itemSub?.land?.certificate_district?.id,
                    certificate_ward: itemSub?.land?.certificate_ward?.id,
                    use_purposes: itemSub?.land?.use_purposes?.map(us => us.id),
                    other_use_purpose: itemSub?.land?.other_use_purpose ?? "",
                    purpose_using_lane: itemSub?.land?.use_purposes?.map(us => us.id),
                    certificate_use_purposes: itemSub?.land?.certificate_use_purposes?.map((fi, index) => ({
                      activeUUIDCertificateUsePurposes: index === 0 ? 'activeUUIDCertificateUsePurposesdepartment' : generateUUID(),
                      use_purpose: fi?.use_purpose,
                      land_number: fi?.land_number,
                      map_number: fi?.map_number,
                      certificate_area: fi?.certificate_area ?? "",
                      real_area: fi?.real_area?.value,
                      land_use_source: fi?.land_use_source?.id,
                      other_land_use_source: fi?.other_land_use_source,
                      duration: fi?.duration,
                      usage_form: fi?.usage_form?.id,
                      other_usage_form: fi?.other_usage_form,
                      apart_land_used_uuid: fi?.apart_land_used_uuid ?? ""
                    })),
                  },
                  department_owner: {
                    active: 0,
                    owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                    owner: itemSub?.owner_wrapper?.owners?.map((ow, owidx) => ({
                      authorized_persons: ow?.authorized_persons?.map(authPer => ({
                        documents: authPer.documents,
                        person_uuid: authPer?.person_uuid,
                        owner_relationship: authPer?.owner_relationship,
                        borrower_relationship: authPer?.borrower_relationship,
                        authorize_contract: authPer?.authorize_contract,
                        owner_auth_uuid: authPer?.owner_auth_uuid
                      })) ?? [],
                      has_authorize: ow?.has_authorize?.id ?? "N",
                      full_name: ow?.full_name,
                      person_uuid: ow?.person_uuid,
                      active: 0,
                      owner_uuid: ow?.owner_uuid
                    })) ?? []
                  },
                  project_name: itemSub?.project_name,
                  has_certificate: itemSub?.has_certificate?.id
                },
                market: {
                  maket_wrapper: {
                    from_credit_extension: itemSub?.from_credit_extension?.id ?? '',
                    is_exploited: itemSub?.is_exploited?.id ?? 'N',
                    credit_extension_secured: itemSub?.credit_extension_secured?.id ?? '',
                    non_business_area: itemSub?.non_business_area?.value.toString() ?? null,
                    max_percentage: itemSub?.max_percentage?.value?.toString() ?? '',
                    value_of_land: itemSub?.collateral_value?.value ?? 0,
                    description: itemSub?.description ?? '',
                    has_land_asset: '',
                    has_certificated_land_asset: itemSub?.has_certificate?.id
                  },
                  maket_certificates: itemSub?.certificates?.map((cer, indexCer) => {
                    return {
                      order: indexCer,
                      persons: cer?.persons?.map(per => ({
                        full_name: per?.full_name,
                        person_uuid: per?.person_uuid,
                        market_cert_item_uuid: per?.market_cert_item_uuid ?? ""
                      })) ?? [],
                      uuid_maket_certificate: indexCer === 0 ? 'maketActiveUuid' : generateUUID(),
                      person_uuid: '',
                      certificate_name: cer.certificate_name ?? '',
                      certificate_code: cer?.certificate_code ?? '',
                      issue_date: cer?.issue_date ? (cer?.issue_date ?? 0) * 1000 : null,
                      place_of_issue: cer.place_of_issue ?? '',
                      contract_name: cer.contract_name ?? '',
                      contract_number: cer.contract_no ?? '',
                      contract_code: cer.contract_code ?? '',
                      contract_date: cer.contract_date ? (cer.contract_date ?? 0) * 1000 : null,
                      contract_unit: cer.contract_unit ?? '',
                      documents: mappingDocument(cer.documents ?? []),
                      market_cert_uuid: cer?.market_cert_uuid ?? ""
                    }
                  }),
                  maket_info: {
                    market_name: itemSub?.market?.market_name ?? '',
                    market_code: itemSub?.market?.market_code ?? '',
                    location: itemSub?.market?.location ?? '',
                    sector: itemSub?.market?.sector ?? '',
                    start_date: itemSub?.market?.start_date ? (itemSub?.market?.start_date ?? 0) * 1000 : null,
                    end_date: itemSub?.market?.end_date ? (itemSub?.market?.end_date ?? 0) * 1000 : null,
                    remaining: itemSub?.market?.remaining?.value?.toString() ?? '',
                    used_area: itemSub?.market?.used_area?.toString() ?? '',
                    value_area: itemSub?.market?.value_area?.value?.toString() ?? '',
                    structure: itemSub?.market?.structure ?? '',
                  },
                  market_owner: {
                    active: 0,
                    owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                    owner: itemSub?.owner_wrapper?.owners.map(ow => ({
                      full_name: ow?.full_name,
                      authorized_persons: ow?.authorized_persons?.map(authPer => ({
                        documents: authPer.documents,
                        person_uuid: authPer?.person_uuid,
                        owner_relationship: authPer?.owner_relationship,
                        borrower_relationship: authPer?.borrower_relationship,
                        authorize_contract: authPer?.authorize_contract,
                        owner_auth_uuid: authPer?.owner_auth_uuid
                      })) ?? [],
                      has_authorize: ow?.has_authorize?.id ?? "N",
                      person_uuid: ow?.person_uuid,
                      active: 0,
                      owner_uuid: ow?.owner_uuid
                    })) ?? []
                  }
                  // ADD MARKET OWNER
                },
                land: {

                  land_wrapper: {  // cuc a 
                    from_credit_extension: itemSub?.land_wrapper?.from_credit_extension?.id ?? 'N',
                    is_exploited: itemSub?.land_wrapper?.is_exploited?.id ?? 'N',
                    credit_extension_secured: itemSub?.land_wrapper?.credit_extension_secured?.id ?? 'N',
                    non_business_area: itemSub?.land_wrapper?.non_business_area?.value?.toString() ?? null,
                    max_percentage: itemSub?.land_wrapper?.max_percentage?.value?.toString() ?? null,
                    value_of_land: itemSub?.land_wrapper?.collateral_value?.value ?? 0,
                    description: itemSub?.land_wrapper?.description,
                    has_land_asset: itemSub?.land_wrapper?.has_land_asset?.id,
                    has_certificated_land_asset: itemSub?.land_wrapper?.has_certificated_land_asset?.id
                  },

                  land_legal_information_owner: {  // thông tin phap ly chu so huu // chekc lai khi co legal // tab cua cuc a
                    active: 0,
                    owner_type: itemSub?.land_wrapper?.owner_wrapper?.owner_type?.id,
                    owner: itemSub?.land_wrapper?.owner_wrapper?.owners?.map(ow => ({
                      full_name: ow?.full_name,
                      authorized_persons: ow?.authorized_persons?.map(authPer => ({
                        documents: authPer.documents,
                        person_uuid: authPer?.person_uuid,
                        owner_relationship: authPer?.owner_relationship,
                        borrower_relationship: authPer?.borrower_relationship,
                        authorize_contract: authPer?.authorize_contract,
                        owner_auth_uuid: authPer?.owner_auth_uuid
                      })) ?? [],
                      has_authorize: ow?.has_authorize?.id ?? "N",
                      person_uuid: ow?.person_uuid,
                      active: 0,
                      owner_uuid: ow?.owner_uuid
                    })) ?? []
                  },

                  land_legal_infomation_asset: { // thông tin dat // tab cua cuc a
                    asset_legal: itemSub?.land_wrapper?.land?.address ?? "",
                    address: itemSub?.land_wrapper?.land?.address ?? "",
                    province: itemSub?.land_wrapper?.land?.province?.id ?? "",
                    district: itemSub?.land_wrapper?.land?.district?.id ?? "",
                    ward: itemSub?.land_wrapper?.land?.ward?.id ?? "",
                    certificate_address: itemSub?.land_wrapper?.land?.certificate_address ?? "",
                    certificate_province: itemSub?.land_wrapper?.land?.certificate_province.id ?? "",
                    certificate_district: itemSub?.land_wrapper?.land?.certificate_district?.id ?? "",
                    certificate_ward: itemSub?.land_wrapper?.land?.certificate_ward?.id ?? "",
                    use_purposes: itemSub?.land_wrapper?.land?.use_purposes?.map(us => us.id),
                    purpose_using_lane: itemSub?.land_wrapper?.land?.use_purposes?.map(us => us.id),
                    purpose_using_lane_other: itemSub?.land_wrapper?.land?.other_use_purpose ?? "",
                    activeUUIDCertificateUsePurposes: generateUUID(),
                    land_asset_types: itemSub?.land_wrapper?.land?.certificate_use_purposes?.map(land => ({
                      activeUUIDCertificateUsePurposes: land?.re_land_used_uuid ?? "",
                      use_purpose: land?.use_purpose ?? "",
                      land_number: land?.land_number ?? "",
                      map_number: land?.map_number ?? "",
                      certificate_area: land?.certificate_area ?? "",
                      real_area: land?.real_area.value ?? "",
                      land_use_source: land?.land_use_source?.id ?? "",
                      other_land_use_source: land?.other_land_use_source ?? "",
                      duration: land?.duration ?? "",
                      usage_form: land?.usage_form.id ?? "",
                      other_usage_form: land?.other_usage_form ?? "",
                      re_land_used_uuid: land?.re_land_used_uuid ?? ""
                    }))
                  },

                  certificate_legal_info: { // thong tin giay chung nhan  // tab cua cuc a
                    activeUUIDCertificate: itemSub?.land_wrapper?.certificates[0]?.land_cert_uuid ?? "",
                    dataCertificate: itemSub?.land_wrapper?.certificates?.map(cerLand => ({
                      persons: cerLand?.persons?.map(per => ({
                        full_name: per?.full_name,
                        person_uuid: per?.person_uuid,
                        re_land_cert_item_uuid: per?.re_land_cert_item_uuid ?? ""
                      })) ?? [],
                      activeUUIDUserListLegal: cerLand?.land_cert_uuid ?? "",
                      activeUUIDCertificateL: cerLand?.land_cert_uuid ?? "",
                      typeUseLand: cerLand?.certificate_type?.id ?? "",
                      typeGCN: cerLand?.other_certificate_type ? cerLand?.other_certificate_type : null,
                      numberGCNLegal: cerLand?.certificate_code ?? "",  // check BE
                      numberGCN: cerLand?.certificate_no ?? "", // check BE
                      dateRange: cerLand?.issue_date ? (cerLand?.issue_date ?? 0) * 1000 : null,
                      dateLocation: cerLand?.place_of_issue ?? "",
                      documents: mappingDocument(cerLand.documents),
                      land_cert_uuid: cerLand?.land_cert_uuid ?? ""
                    }))
                  },
                },
                ctxd_land: {  // cuc B 
                  activeCTXDLand: 'activeCTXDLand',  // check active
                  dataCTXDLand: itemSub?.land_asset_wrapper?.land_assets?.map((ctxd, ctxdIndex) => ({
                    activeUUIDCTXDLand: ctxdIndex === 0 ? 'activeCTXDLand' : generateUUID(), // check active
                    asset_legal: ctxd?.asset_legal?.id ?? "",
                    legal_CTXD_other: ctxd?.other_asset_legal ?? null,
                    address: ctxd?.address ?? "",
                    provice: ctxd?.province?.id ?? "",
                    district: ctxd?.district?.id ?? "",
                    ward: ctxd?.ward?.id ?? "",
                    certificate_address: ctxd?.certificate_address ?? "",
                    certificate_province: ctxd?.certificate_province?.id ?? "",
                    certificate_district: ctxd?.certificate_district?.id ?? "",
                    certificate_ward: ctxd?.certificate_ward?.id ?? "",
                    activeUUIDtypeCTXD: 'activeUUIDtypeCTXD', // check active
                    documents: mappingDocument(ctxd?.documents ?? []),
                    land_const_item_uuid: ctxd?.land_const_item_uuid,
                    dataTypeCTXD: ctxd?.land_asset_types?.map((ctxdType, ctxdTypeIndex) => ({
                      activeTypeCTXD: ctxdTypeIndex === 0 ? 'activeUUIDtypeCTXD' : generateUUID(),// check active
                      land_asset_type: ctxdType?.land_asset_type?.id ?? "",
                      land_asset_type_other: ctxdType?.other_asset_type ?? "",
                      certificate_building_area: ctxdType?.certificate_building_area?.toString() ?? "",
                      building_area: ctxdType?.building_area?.value?.toString() ?? "",
                      certificate_cross_floor_area: ctxdType?.certificate_cross_floor_area?.toString() ?? "",
                      cross_floor_area: ctxdType?.cross_floor_area?.value?.toString() ?? "",
                      certificate_used_area: ctxdType?.certificate_used_area?.toString() ?? "",
                      used_area: ctxdType?.used_area?.value?.toString() ?? "",
                      ownership_duration: ctxdType?.ownership_duration ?? "",
                      owner_form: ctxdType?.owner_form ?? "",
                      certificate_structure: ctxdType?.certificate_structure ?? "",
                      structure: ctxdType?.structure ?? "",
                      certificate_rank: ctxdType?.certificate_rank ?? "",
                      certificate_floors: ctxdType?.certificate_floors ?? "",
                      floors: ctxdType?.floors ?? "",
                      duration_of_use: ctxdType?.duration_of_use ?? "",
                      land_const_item_detail_uuid: ctxdType?.land_const_item_detail_uuid ?? ""
                    })) ?? []
                  })) ?? [],
                  ctx_land_wrapper: {  // A. THÔNG TIN ĐỊNH GIÁ VÀ THẨM ĐỊNH TÀI SẢN cuc B 
                    from_credit_extension: itemSub?.land_asset_wrapper?.from_credit_extension?.id ?? 'N',
                    is_exploited: itemSub?.land_asset_wrapper?.is_exploited?.id ?? 'N',
                    credit_extension_secured: itemSub?.land_asset_wrapper?.credit_extension_secured?.id ?? 'N',
                    non_business_area: itemSub?.land_asset_wrapper?.non_business_area?.value?.toString() ?? null,
                    max_percentage: itemSub?.land_asset_wrapper?.max_percentage?.value?.toString() ?? null,
                    value_of_land: itemSub?.land_asset_wrapper?.collateral_value?.value ?? "",
                    description: itemSub?.land_asset_wrapper?.description ?? "",
                    has_land_asset: "",
                    has_certificated_land_asset: "",
                    land_const_uuid: itemSub?.land_asset_wrapper?.land_const_uuid
                  }
                },
                ctxd_gcn_qsh: { // cuc C
                  activeUuIdCtxdGcnQsh: 'activeUuIdCtxdGcnQsh',
                  ctxd_gcn_qsh_data: itemSub?.certificated_land_asset_wrappers?.map((ctxd_gcn, idxctxd_gcn) => ({
                    land_const_uuid: ctxd_gcn.land_const_uuid,
                    uuIdCtxdGcnQsh: idxctxd_gcn === 0 ? 'activeUuIdCtxdGcnQsh' : generateUUID(),
                    documents: mappingDocument(ctxd_gcn?.documents ?? []),
                    land_legal_information_owner: {   // thong tin tab A 
                      active: 0,
                      owner_type: ctxd_gcn?.owner_wrapper?.owner_type?.id,
                      owner: ctxd_gcn?.owner_wrapper?.owners?.map((ow) => ({
                        authorized_persons: ow?.authorized_persons?.map(authPer => ({
                          documents: authPer.documents,
                          person_uuid: authPer?.person_uuid,
                          owner_relationship: authPer?.owner_relationship,
                          borrower_relationship: authPer?.borrower_relationship,
                          authorize_contract: authPer?.authorize_contract,
                          owner_auth_uuid: authPer?.owner_auth_uuid
                        })) ?? [],
                        full_name: ow?.full_name,
                        has_authorize: ow?.has_authorize?.id ?? "N",
                        person_uuid: ow?.person_uuid,
                        active: 0,
                        owner_uuid: ow?.owner_uuid
                      }))
                    },
                    certificate_legal_info: { // thong tin tab B
                      activeUUIDCertificate: ctxd_gcn?.certificates[0]?.land_const_item_cert_uuid ?? "",
                      dataCertificate: ctxd_gcn?.certificates?.map(cer => ({
                        persons: cer?.persons?.map(per => ({
                          full_name: per?.full_name,
                          person_uuid: per?.person_uuid,
                          land_const_item_cert_item_uuid: per?.land_const_item_cert_item_uuid ?? ""
                        })),
                        documents: mappingDocument(cer.documents),
                        activeUUIDUserListLegal: generateUUID(),
                        activeUUIDCertificateL: cer?.land_const_item_cert_uuid ?? "",
                        typeUseLand: cer?.certificate_type?.id ?? "",
                        typeGCN: cer?.other_certificate_type ?? "",
                        numberGCNLegal: cer?.certificate_code ?? "",
                        numberGCN: cer?.certificate_no ?? "",
                        dateRange: cer?.issue_date ? (cer?.issue_date ?? 0) * 1000 : null,
                        dateLocation: cer?.place_of_issue ?? "",
                        land_const_item_cert_uuid: cer?.land_const_item_cert_uuid ?? ""
                      }))
                    },

                    ctxd_gcn_qsh_land_info: {  // thong tin tab C

                      dataCTXDLand: {
                        activeUUIDCTXDLand: generateUUID(),
                        asset_legal: ctxd_gcn?.land_asset?.asset_legal?.id ?? "",
                        legal_CTXD_other: ctxd_gcn?.land_asset?.other_asset_legal ?? null,
                        address: ctxd_gcn?.land_asset?.address ?? "",
                        provice: ctxd_gcn?.land_asset?.province?.id ?? "",
                        district: ctxd_gcn?.land_asset?.district?.id ?? "",
                        ward: ctxd_gcn?.land_asset?.ward?.id ?? "",
                        certificate_address: ctxd_gcn?.land_asset?.certificate_address ?? "",
                        certificate_province: ctxd_gcn?.land_asset?.certificate_province?.id ?? "",
                        certificate_district: ctxd_gcn?.land_asset?.certificate_district?.id ?? "",
                        certificate_ward: ctxd_gcn?.land_asset?.certificate_ward?.id ?? "",
                        activeUUIDtypeCTXD: 'activeUUIDtypeCTXD',
                        dataTypeCTXD: ctxd_gcn?.land_asset?.land_asset_types?.map((typed, indexType) => ({
                          activeTypeCTXD: indexType === 0 ? 'activeUUIDtypeCTXD' : generateUUID(),
                          land_asset_type: typed?.land_asset_type?.id ?? "",
                          land_asset_type_other: typed?.other_asset_type ?? "",
                          certificate_building_area: typed?.certificate_building_area?.toString() ?? "",
                          building_area: typed?.building_area?.value?.toString() ?? "",
                          certificate_cross_floor_area: typed?.certificate_cross_floor_area?.toString() ?? "",
                          cross_floor_area: typed?.cross_floor_area?.value?.toString() ?? "",
                          certificate_used_area: typed?.certificate_used_area?.toString() ?? "",
                          used_area: typed?.used_area?.value?.toString() ?? "",
                          ownership_duration: typed?.ownership_duration ?? "",
                          owner_form: typed?.owner_form ?? "",
                          certificate_structure: typed?.certificate_structure ?? "",
                          structure: typed?.structure ?? "",
                          certificate_rank: typed?.certificate_rank ?? "",
                          certificate_floors: typed?.certificate_floors ?? "",
                          floors: typed?.floors ?? "",
                          duration_of_use: typed?.duration_of_use ?? "",
                          land_const_item_detail_uuid: typed?.land_const_item_detail_uuid ?? ""
                        })) ?? []
                      },
                      ctx_land_wrapper: {  // A. THÔNG TIN ĐỊNH GIÁ VÀ THẨM ĐỊNH TÀI SẢN  CHECK LAI UX 
                        from_credit_extension: ctxd_gcn?.from_credit_extension?.id ?? "",
                        is_exploited: ctxd_gcn?.is_exploited?.id ?? "N",
                        credit_extension_secured: ctxd_gcn?.credit_extension_secured?.id ?? "",
                        non_business_area: ctxd_gcn?.non_business_area?.value?.toString() ?? null,
                        max_percentage: ctxd_gcn?.max_percentage?.value?.toString() ?? null,
                        value_of_land: ctxd_gcn?.collateral_value?.value ?? "",
                        description: ctxd_gcn?.description ?? "",
                        has_land_asset: "",
                        has_certificated_land_asset: "",
                      },
                      activeCTXDLand: generateUUID(),
                    }

                  })) ?? []
                },
                price_cert_asset_uuid: itemSub?.price_cert_asset_uuid ?? "",

              }
            }) as unknown as ISubItems[]
          }
        }) as ISubtype[],
        price_cert_uuid: item?.price_cert_uuid
      }
    }) as ILOANNormalCollateralData[]
    state.storage.collateral_v2.originData = state.storage.collateral_v2.data
  },


  fetchDataUpdateAPIStorageCollateralType: { // fetch obj collateral
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<ICollateralFormGet, string, string>
    ) {
      if (state.storage.full.data) {
        if (state.storage.full.data.form.collateral_form) {
          state.storage.full.data.form.collateral_form.data.collaterals = state.storage.full.data.form.collateral_form.data.collaterals.map(i => {
            if (i.price_cert_uuid === action.payload.price_cert_uuid) {
              i = action.payload
            }
            return { ...i }
          })
        }
      }
      const mappingDocument = (data: IResDocument[]) => {
        if (!data) return [];
        return data.map((doc: IResDocument) => {
          // 
          const result: ILOANNormalDocument = {
            uuid: generateUUID(),
            document_id: doc.document_id,
            document_name: doc.document_name,
            child_files: doc?.child_files?.map((file, index) => {
              const result: ILOANNormalFile = {
                content_type: file.content_type,
                created_at: file.created_at,
                created_by: file.created_by,
                created_by_name: file.created_by_name,
                updated_at: file.updated_at,
                updated_by: file.updated_by,
                updated_by_name: file.updated_by_name,      
                description: file.description,
                display_order: index + 1,
                file_id: index,
                name: file.name,
                uuid: file.uuid,
                custom_keys: file.custom_keys,
                file_upload: '',
              };
              return result;
            }) ?? []
          }
          return result;
        });
      };

      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map((item) => {
        if (item.is_collapse_type === true) {
          const dataActiveUUID = generateUUID();
          const SubtypeActiveUUID = generateUUID();
          state.storage.collateral_v2.uuidActiveData = dataActiveUUID;
          return {
            carousel: [],
            uuidActiveData: dataActiveUUID,
            isSaved: true,
            uuidActiveSubtype: SubtypeActiveUUID,
            is_collapse_type: true,
            type: action.payload?.collateral_type?.id,
            status: action.payload?.status?.id,
            is_compactness: action.payload?.is_compactness?.id,
            valuation_id: action.payload?.valuation_id,
            valuation_date: action.payload?.valuation_date ? (action.payload?.valuation_date) * 1000 : null,
            valuation_unit_type: action.payload?.valuation_unit_type?.id,
            valuation_unit: action.payload?.valuation_unit?.id,
            valuation_center: action.payload?.valuation_center?.id,
            valuation_center_opinion: action.payload?.valuation_center_opinion,
            independence_organization: action.payload?.independence_organization?.id,
            other_independence_organization: action.payload?.other_independence_organization,
            purpose: action.payload?.purpose?.id,
            other_purpose: action.payload?.other_purpose,
            address: action.payload?.address,
            province: action.payload?.province?.id,
            district: action.payload?.district?.id,
            ward: action.payload?.ward?.id,
            position_type: action.payload?.position_type?.id,
            other_position_type: action.payload?.other_position_type,
            lane_width: action.payload?.lane_width?.id,
            description: action.payload?.description,
            collateral_value: action.payload?.collateral_value.value,
            max_percentage: action.payload?.max_percentage?.value,
            documents: mappingDocument(action.payload.documents ?? []),
            sub_type: action.payload?.sub_types && action.payload?.sub_types?.map(i => {
              const result = {
                is_collapse_sub_type: false,
                uuidItemsActive: item.sub_type[0]?.uuidItemsActive ?? "",
                uuidActiveSubtype: SubtypeActiveUUID,
                id: i.id,
                child_sub_type: i.child_id,
                items: i.items && i.items.map((itemSub, indexItem) => {
                    const indexActiveMarketCer = item?.sub_type[0]?.items[indexItem]?.market?.maket_certificates?.findIndex(i=>i.uuid_maket_certificate === item.sub_type[0]?.items[indexItem]?.maketActiveUuid) ?? 0
                    
                    const indexActiveCtxd = item.sub_type[0]?.items[indexItem]?.ctxd_land?.dataCTXDLand.findIndex(i=> i.activeUUIDCTXDLand === item.sub_type[0]?.items[indexItem]?.ctxd_land?.activeCTXDLand) ?? 0
                    const indexActiveTypeCtxd = item.sub_type[0]?.items[indexItem]?.ctxd_land?.dataCTXDLand[indexActiveCtxd]?.dataTypeCTXD?.findIndex(k => k.activeTypeCTXD === item.sub_type[0]?.items[indexItem]?.ctxd_land?.dataCTXDLand[indexActiveCtxd]?.activeUUIDtypeCTXD) ?? 0
                    const activeCTXDGCNQSH = item.sub_type[0]?.items[indexItem]?.ctxd_gcn_qsh.ctxd_gcn_qsh_data.find(i => 
                    i.uuIdCtxdGcnQsh === item.sub_type[0]?.items[indexItem]?.ctxd_gcn_qsh?.activeUuIdCtxdGcnQsh)
                    const indexActiveCTXDGCNQSH = item?.sub_type[0]?.items[indexItem]?.ctxd_gcn_qsh?.ctxd_gcn_qsh_data?.findIndex(o => o.uuIdCtxdGcnQsh === item?.sub_type[0]?.items[indexItem]?.ctxd_gcn_qsh?.activeUuIdCtxdGcnQsh) ?? 0
                    const indexActiveCerCTXDGCNQSH = activeCTXDGCNQSH?.certificate_legal_info.dataCertificate.findIndex(o=>
                      o.activeUUIDCertificateL === activeCTXDGCNQSH?.certificate_legal_info.activeUUIDCertificate) ?? 0
                    return {
                    activeUUID: item?.sub_type[0]?.items[indexItem]?.activeUUID ?? "",
                    type_land: item?.sub_type[0]?.items?.find(itx => itx.activeUUID === item.sub_type[0]?.uuidItemsActive)?.type_land ?? "",
                    departmentActiveUUID: item.sub_type[0]?.items[indexItem]?.departmentActiveUUID ?? "",
                    maketActiveUuid: itemSub?.certificates ? itemSub?.certificates[indexActiveMarketCer]?.market_cert_uuid ?? ""  : "",
                    // maketActiveUuid: !!indexActiveMarketCer && indexActiveMarketCer !== -1 ? itemSub?.certificates[indexActiveMarketCer]?.market_cert_uuid  : "",
                    current_value_item: itemSub?.collateral_value?.value,
                    ratio: itemSub?.max_percentage?.value,
                    value: itemSub?.collateral_value?.value,
                    typeCollateral: i.id === "BALC" ? itemSub?.detail?.balance_type?.id : itemSub?.detail?.name,
                    license: itemSub?.detail?.license_number,
                    status: i?.id === 'BALC' || i?.id === 'STOC' || i?.id === 'OTHE' ? itemSub?.detail?.status as string : itemSub?.detail?.status?.id,
                    status_flag: itemSub?.detail?.status,
                    description: itemSub?.detail?.description,
                    collateral_value: itemSub?.collateral_value?.value,
                    collateral_code: itemSub?.collateral_code,
                    count: itemSub?.detail?.quantity_each_type,
                    branch: itemSub?.detail?.brand,
                    brand: itemSub?.detail?.brand,
                    other_brand: itemSub?.detail?.other_brand,
                    model: itemSub?.detail?.model,
                    year: Number(itemSub?.detail?.manufacturing_date),
                    info_collatetal: itemSub?.description,
                    quantity: itemSub?.detail?.quantity,
                    number_register: itemSub?.detail?.license_number,
                    issuer: itemSub?.detail?.distributor?.id,
                    other_issuer: itemSub?.detail?.other_distributor,
                    CLCL: itemSub?.detail?.remain_quality?.value,
                    production: itemSub?.detail?.origin_of_production,
                    origin_of_production: itemSub?.detail?.origin_of_production?.id,
                    other_origin_of_production: itemSub?.detail?.other_origin_of_production,
                    vehicle_identification_number: itemSub?.detail?.vehicle_identification_number ?? "",
                    license_number: itemSub?.detail?.vehicle_identification_number ?? "",
                    engine_number: itemSub?.detail?.engine_number ?? "",
                    license_plate: itemSub?.detail?.license_plate ?? "",
                    departmentInfoActiveUUID: item.sub_type[0]?.items[indexItem]?.departmentInfoActiveUUID ?? "",
                    has_certificate_maket: itemSub?.has_certificate?.id,
                    documents:mappingDocument(itemSub?.detail?.documents ?? []),
                    transportation_sub_type: itemSub?.detail?.transportation_sub_type,
                    other_transportation_sub_type: itemSub?.detail?.other_transportation_sub_type,
                    legal_document_types: itemSub?.detail?.legal_document_types,
                    credit_extension_secured: itemSub?.credit_extension_secured?.id,
                    owner_wrapper: {
                      owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                      owner: itemSub?.owner_wrapper?.owners?.map(ow => ({
                        full_name: ow?.full_name,
                        person_uuid: ow?.person_uuid,
                        owner_uuid: ow?.owner_uuid ?? ""
                      })) ?? []
                    },
                    activeUUIDCertificateUsePurposes: item.sub_type[0]?.items[indexItem]?.activeUUIDCertificateUsePurposes ?? "",
                    department: {   // type chung cu 
                      department_wrapper: {
                        from_credit_extension: itemSub?.from_credit_extension?.id,
                        is_exploited: itemSub?.is_exploited?.id ?? 'N',
                        credit_extension_secured: itemSub?.credit_extension_secured?.id,
                        non_business_area: itemSub?.non_business_area?.value?.toString() ?? null,
                        max_percentage: itemSub?.max_percentage?.value?.toString(),
                        value_of_land: itemSub?.collateral_value?.value, // check ux or API
                        description: itemSub?.description,
                        has_land_asset: itemSub?.has_certificate?.id,// check ux or API
                        has_certificated_land_asset: itemSub?.has_certificate?.id,// check ux or API
                      },
                      department_certificate_legal: itemSub?.certificates?.map((cerr, idx) => ({
                        order: idx + 1,
                        persons: cerr?.persons?.map(per => ({
                          full_name: per?.full_name,
                          person_uuid: per?.person_uuid,
                          apart_owner_cert_item_uuid: per?.apart_owner_cert_item_uuid ?? ""
                        })) ?? [], // check legal
                        uuid_certificate_legal: item.sub_type[0]?.items[indexItem]?.department?.department_certificate_legal[idx]?.uuid_certificate_legal ?? "",
                        other_certificate_type: cerr?.certificate_type?.id,  // check 
                        other_certificate_type_other: cerr?.other_certificate_type, // check
                        certificate_code: cerr?.certificate_code,
                        certificate_no: cerr?.certificate_no,
                        issue_date: cerr?.issue_date ? (cerr?.issue_date ?? 0) * 1000 : null,
                        place_of_issue: cerr?.place_of_issue,
                        contract_type: cerr?.contract_type,
                        contract_number_type: cerr?.contract_type,
                        contract_number: cerr?.contract_no,
                        contract_date: cerr?.contract_date ? (cerr?.contract_date ?? 0) * 1000 : null,
                        documents: mappingDocument(cerr?.documents ?? []),
                        apart_owner_cert_uuid: cerr?.apart_owner_cert_uuid ?? ""
                      })),
                      department_info: itemSub?.apartments?.map((depp, ii) => {
                        return {
                          departmentInfoActiveUUID: item.sub_type[0]?.items[indexItem]?.department?.department_info[ii]?.departmentInfoActiveUUID ?? "",
                          house_type: depp?.house_type,
                          apartment_type: depp?.apartment_type?.id,
                          other_apartment_type: depp?.other_apartment_type,
                          apartment_number: depp?.apartment_number,
                          block: depp?.block,
                          floor: depp?.floor,
                          start_date: depp?.start_date ? (depp?.start_date ?? 0) * 1000 : null,
                          certificate_area: depp?.certificate_area ?? "",
                          real_area: depp?.real_area.value,
                          usage_form: depp?.usage_from,
                          duration: depp?.duration,
                          ownership_category: depp?.ownership_category,
                          apart_room_uuid:depp?.apart_room_uuid ?? ""
                        }
                      }),
                      department_info_land: {
                        address: itemSub?.land?.address,
                        province: itemSub?.land?.province?.id,
                        district: itemSub?.land?.district?.id,
                        ward: itemSub?.land?.ward?.id,
                        certificate_address: itemSub?.land?.certificate_address,
                        certificate_province: itemSub?.land?.certificate_province?.id,
                        certificate_district: itemSub?.land?.certificate_district?.id,
                        certificate_ward: itemSub?.land?.certificate_ward?.id,
                        use_purposes: itemSub?.land?.use_purposes?.map(us => us.id),
                        other_use_purpose: itemSub?.land?.other_use_purpose ?? "",
                        purpose_using_lane: itemSub?.land?.use_purposes?.map(us => us.id),
                        purpose_using_lane_other: itemSub?.land_wrapper?.land?.other_use_purpose ?? "",
                        certificate_use_purposes: itemSub?.land?.certificate_use_purposes?.map((fi, indexUse) => ({
                          activeUUIDCertificateUsePurposes: item.sub_type[0]?.items[indexItem]?.department?.department_info_land?.certificate_use_purposes[indexUse]?.activeUUIDCertificateUsePurposes ?? "",
                          use_purpose: fi?.use_purpose,
                          land_number: fi?.land_number,
                          map_number: fi?.map_number,
                          certificate_area: fi?.certificate_area ?? "",
                          real_area: fi?.real_area?.value,
                          land_use_source: fi?.land_use_source?.id,
                          other_land_use_source: fi?.other_land_use_source,
                          duration: fi?.duration,
                          usage_form: fi?.usage_form?.id,
                          other_usage_form: fi?.other_usage_form,
                          apart_land_used_uuid: fi?.apart_land_used_uuid ?? ""
                        })),
                      },
                      department_owner: {
                        active: item.sub_type[0]?.items[indexItem]?.department?.department_owner?.active ?? 0,
                        owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                        owner: itemSub?.owner_wrapper?.owners?.map((ow, owidx) => {
                          const activeAuthor = item.sub_type[0]?.items[indexItem]?.department?.department_owner.owner.find(
                            auth => auth.person_uuid === ow.person_uuid
                          )?.active
                         return {
                          authorized_persons: ow?.authorized_persons?.map(authPer => ({
                            documents: authPer.documents,
                            person_uuid: authPer?.person_uuid,
                            owner_relationship: authPer?.owner_relationship,
                            borrower_relationship: authPer?.borrower_relationship,
                            authorize_contract: authPer?.authorize_contract,
                            owner_auth_uuid: authPer?.owner_auth_uuid
                          })) ?? [],
                          has_authorize: ow?.has_authorize?.id ?? "N",
                          full_name: ow?.full_name,
                          person_uuid: ow?.person_uuid,
                          active: activeAuthor ?? 0,
                          owner_uuid: ow?.owner_uuid
                        }}) ?? []
                      },
                      project_name: itemSub?.project_name,
                      has_certificate: itemSub?.has_certificate?.id
                    },
                    market: {
                      maket_wrapper: {
                        from_credit_extension: itemSub?.from_credit_extension?.id ?? '',
                        is_exploited: itemSub?.is_exploited?.id ?? 'N',
                        credit_extension_secured: itemSub?.credit_extension_secured?.id ?? '',
                        non_business_area: itemSub?.non_business_area?.value?.toString() ?? null,
                        max_percentage: itemSub?.max_percentage?.value?.toString() ?? null,
                        value_of_land: itemSub?.collateral_value?.value ?? 0,
                        description: itemSub?.description ?? '',
                        has_land_asset: '',
                        has_certificated_land_asset: itemSub?.has_certificate?.id
                      },
                      maket_certificates: itemSub?.certificates?.map((cer, indexCer) => {
                        return {
                          order: cer?.order ?? 1,
                          persons: cer?.persons?.map(per => ({
                            full_name: per?.full_name,
                            person_uuid: per?.person_uuid,
                            market_cert_item_uuid: per?.market_cert_item_uuid ?? ""
                          })) ?? [],
                          uuid_maket_certificate: cer?.market_cert_uuid ?? "",
                          certificate_name: cer?.certificate_name ?? '',
                          certificate_code: cer?.certificate_code ?? '',
                          issue_date: cer?.issue_date ? (cer?.issue_date ?? 0) * 1000 : null,
                          place_of_issue: cer?.place_of_issue ?? '',
                          contract_name: cer?.contract_name ?? '',
                          contract_number: cer?.contract_no ?? '',
                          contract_code: cer?.contract_code ?? '',
                          contract_date: cer?.contract_date ? (cer?.contract_date ?? 0) * 1000 : null,
                          contract_unit: cer?.contract_unit ?? '',
                          documents: mappingDocument(cer?.documents ?? []),
                          market_cert_uuid: cer?.market_cert_uuid ?? ""
                        }
                      }),
                      maket_info: {
                        market_name: itemSub?.market?.market_name ?? '',
                        market_code: itemSub?.market?.market_code ?? '',
                        location: itemSub?.market?.location ?? '',
                        sector: itemSub?.market?.sector ?? '',
                        start_date: itemSub?.market?.start_date ? (itemSub?.market?.start_date ?? 0) * 1000 : null,
                        end_date: itemSub?.market?.start_date ? (itemSub?.market?.end_date ?? 0) * 1000 : null,
                        remaining: itemSub?.market?.remaining?.value?.toString() ?? '',
                        used_area: itemSub?.market?.used_area?.toString() ?? '',
                        value_area: itemSub?.market?.value_area?.value?.toString() ?? '',
                        structure: itemSub?.market?.structure ?? '',
                      },
                      market_owner: {
                        active: item.sub_type[0]?.items[indexItem]?.market?.market_owner?.active ?? 0,
                        owner_type: itemSub?.owner_wrapper?.owner_type?.id,
                        owner: itemSub?.owner_wrapper?.owners.map(ow => {
                          const authActive =  item.sub_type[0]?.items[indexItem]?.market?.market_owner?.owner?.find(auth => auth.person_uuid === ow.person_uuid)?.active
                          return {
                          full_name: ow?.full_name,
                          authorized_persons: ow?.authorized_persons?.map(authPer => ({
                            documents: authPer.documents,
                            person_uuid: authPer?.person_uuid,
                            owner_relationship: authPer?.owner_relationship,
                            borrower_relationship: authPer?.borrower_relationship,
                            authorize_contract: authPer?.authorize_contract,
                            owner_auth_uuid: authPer?.owner_auth_uuid
                          })) ?? [],
                          has_authorize: ow?.has_authorize?.id ?? "N",
                          person_uuid: ow?.person_uuid,
                          active: authActive ?? 0,
                          owner_uuid: ow?.owner_uuid
                        }}) ?? []
                      }
                      // ADD MARKET OWNER
                    },
                    land: {
                      land_wrapper: {  // cuc a 
                        from_credit_extension: itemSub?.land_wrapper?.from_credit_extension?.id ?? 'N',
                        is_exploited: itemSub?.land_wrapper?.is_exploited?.id ?? 'N',
                        credit_extension_secured: itemSub?.land_wrapper?.credit_extension_secured?.id ?? 'N',
                        non_business_area: itemSub?.land_wrapper?.non_business_area?.value?.toString() ?? null,
                        max_percentage: itemSub?.land_wrapper?.max_percentage?.value?.toString() ?? null,
                        value_of_land: itemSub?.land_wrapper?.collateral_value?.value ?? 0,
                        description: itemSub?.land_wrapper?.description,
                        has_land_asset: itemSub?.land_wrapper?.has_land_asset?.id,
                        has_certificated_land_asset: itemSub?.land_wrapper?.has_certificated_land_asset?.id
                      },

                      land_legal_information_owner: {  // thông tin phap ly chu so huu // chekc lai khi co legal // tab cua cuc a
                        active: item.sub_type[0]?.items[indexItem]?.land?.land_legal_information_owner?.active ?? 0,
                        owner_type: itemSub?.land_wrapper?.owner_wrapper?.owner_type?.id,
                        owner: itemSub?.land_wrapper?.owner_wrapper?.owners?.map(ow => {
                          const authActive = item.sub_type[0]?.items[indexItem]?.land?.land_legal_information_owner?.owner?.find(
                            au => au.person_uuid === ow.person_uuid
                          )?.active
                          return {
                          full_name: ow?.full_name,
                          authorized_persons: ow?.authorized_persons?.map(authPer => ({
                            documents: authPer.documents,
                            person_uuid: authPer?.person_uuid,
                            owner_relationship: authPer?.owner_relationship,
                            borrower_relationship: authPer?.borrower_relationship,
                            authorize_contract: authPer?.authorize_contract,
                            owner_auth_uuid: authPer?.owner_auth_uuid
                          })) ?? [],
                          has_authorize: ow?.has_authorize?.id ?? "N",
                          person_uuid: ow?.person_uuid,
                          active: authActive ?? 0,
                          owner_uuid: ow?.owner_uuid
                        }}) ?? []
                      },

                      land_legal_infomation_asset: { // thông tin dat // tab cua cuc a
                        asset_legal: itemSub?.land_wrapper?.land?.address ?? "",
                        address: itemSub?.land_wrapper?.land?.address ?? "",
                        province: itemSub?.land_wrapper?.land?.province?.id ?? "",
                        district: itemSub?.land_wrapper?.land?.district?.id ?? "",
                        ward: itemSub?.land_wrapper?.land?.ward?.id ?? "",
                        certificate_address: itemSub?.land_wrapper?.land?.certificate_address ?? "",
                        certificate_province: itemSub?.land_wrapper?.land?.certificate_province.id ?? "",
                        certificate_district: itemSub?.land_wrapper?.land?.certificate_district?.id ?? "",
                        certificate_ward: itemSub?.land_wrapper?.land?.certificate_ward?.id ?? "",
                        use_purposes: itemSub?.land_wrapper?.land.use_purposes?.map(us => us.id),
                        purpose_using_lane: itemSub?.land_wrapper?.land.use_purposes?.map(us => us.id),
                        purpose_using_lane_other: itemSub?.land_wrapper?.land?.other_use_purpose ?? "",
                        activeUUIDCertificateUsePurposes: 'activeUUIDCertificateUsePurposes',
                        land_asset_types: itemSub?.land_wrapper?.land?.certificate_use_purposes?.map((land, idx) => ({
                          activeUUIDCertificateUsePurposes: idx === 0 ? 'activeUUIDCertificateUsePurposes' : generateUUID(),
                          use_purpose: land?.use_purpose ?? "",
                          land_number: land?.land_number ?? "",
                          map_number: land?.map_number ?? "",
                          certificate_area: land?.certificate_area ?? "",
                          real_area: land?.real_area.value ?? "",
                          land_use_source: land?.land_use_source?.id ?? "",
                          other_land_use_source: land?.other_land_use_source ?? "",
                          duration: land?.duration ?? "",
                          usage_form: land?.usage_form.id ?? "",
                          other_usage_form: land?.other_usage_form ?? "",
                          re_land_used_uuid: land?.re_land_used_uuid ?? ""
                        }))
                      },

                      certificate_legal_info: { // thong tin giay chung nhan  // tab cua cuc a
                        activeUUIDCertificate: item.sub_type[0]?.items[indexItem].land.certificate_legal_info.activeUUIDCertificate,
                        dataCertificate: itemSub?.land_wrapper?.certificates?.map((cerLand,cerLandIndex) => ({
                          persons: cerLand?.persons?.map(per => ({
                            full_name: per?.full_name,
                            person_uuid: per?.person_uuid,
                            re_land_cert_item_uuid: per?.re_land_cert_item_uuid ?? ""
                          })) ?? [],
                          activeUUIDUserListLegal: cerLand?.land_cert_uuid ?? "",
                          activeUUIDCertificateL: item.sub_type[0]?.items[indexItem].land.certificate_legal_info.dataCertificate[cerLandIndex].activeUUIDCertificateL ?? generateUUID(),
                          typeUseLand: cerLand?.certificate_type?.id ?? "",
                          typeGCN: cerLand?.other_certificate_type ?? "",
                          numberGCNLegal: cerLand?.certificate_code ?? "",  // check BE
                          numberGCN: cerLand?.certificate_no ?? "", // check BE
                          dateRange: cerLand?.issue_date ? (cerLand?.issue_date ?? 0) * 1000 : null,
                          dateLocation: cerLand?.place_of_issue ?? "",
                          documents: mappingDocument(cerLand.documents),
                          land_cert_uuid: cerLand?.land_cert_uuid ?? ""
                        }))
                      },

                    },

                    ctxd_land: {  // cuc B 
                      activeCTXDLand: itemSub?.land_asset_wrapper?.land_assets[indexActiveCtxd]?.land_const_item_uuid ?? "",  // check active
                      
                      dataCTXDLand: itemSub?.land_asset_wrapper?.land_assets?.map((ctxd, ctxdIndex) => ({
                        activeUUIDCTXDLand: ctxd?.land_const_item_uuid ?? "", // check active
                        asset_legal: ctxd?.asset_legal?.id ?? "",
                        legal_CTXD_other: ctxd?.other_asset_legal ? ctxd?.other_asset_legal : null,
                        address: ctxd?.address ?? "",
                        provice: ctxd?.province?.id ?? "",
                        district: ctxd?.district?.id ?? "",
                        ward: ctxd?.ward?.id ?? "",
                        certificate_address: ctxd?.certificate_address ?? "",
                        certificate_province: ctxd?.certificate_province?.id ?? "",
                        certificate_district: ctxd?.certificate_district?.id ?? "",
                        certificate_ward: ctxd?.certificate_ward?.id ?? "",
                        activeUUIDtypeCTXD: ctxd?.land_asset_types[indexActiveTypeCtxd]?.land_const_item_detail_uuid ?? "", // check active
                        documents: mappingDocument(ctxd?.documents ?? []),
                        land_const_item_uuid: ctxd?.land_const_item_uuid,
                        dataTypeCTXD: ctxd?.land_asset_types?.map((ctxdType) => ({
                          activeTypeCTXD: ctxdType?.land_const_item_detail_uuid ?? "",// check active
                          land_asset_type: ctxdType?.land_asset_type.id ?? "",
                          land_asset_type_other: ctxdType?.other_asset_type ?? "",
                          certificate_building_area: ctxdType?.certificate_building_area?.toString() ?? "",
                          building_area: ctxdType?.building_area?.value?.toString() ?? "",
                          certificate_cross_floor_area: ctxdType?.certificate_cross_floor_area?.toString() ?? "",
                          cross_floor_area: ctxdType?.cross_floor_area?.value?.toString() ?? "",
                          certificate_used_area: ctxdType?.certificate_used_area?.toString() ?? "",
                          used_area: ctxdType?.used_area?.value?.toString() ?? "",
                          ownership_duration: ctxdType?.ownership_duration ?? "",
                          owner_form: ctxdType?.owner_form ?? "",
                          certificate_structure: ctxdType?.certificate_structure ?? "",
                          structure: ctxdType?.structure ?? "",
                          certificate_rank: ctxdType?.certificate_rank ?? "",
                          certificate_floors: ctxdType?.certificate_floors ?? "",
                          floors: ctxdType?.floors ?? "",
                          duration_of_use: ctxdType?.duration_of_use ?? "",
                          land_const_item_detail_uuid: ctxdType?.land_const_item_detail_uuid ?? ""
                        })) ?? []
                      })) ?? [],
                      ctx_land_wrapper: {  // A. THÔNG TIN ĐỊNH GIÁ VÀ THẨM ĐỊNH TÀI SẢN cuc B 
                        from_credit_extension: itemSub?.land_asset_wrapper?.from_credit_extension?.id ?? 'N',
                        is_exploited: itemSub?.land_asset_wrapper?.is_exploited?.id ?? 'N',
                        credit_extension_secured: itemSub?.land_asset_wrapper?.credit_extension_secured?.id ?? 'N',
                        non_business_area: itemSub?.land_asset_wrapper?.non_business_area?.value?.toString() ?? null,
                        max_percentage: itemSub?.land_asset_wrapper?.max_percentage?.value?.toString() ?? null,
                        value_of_land: itemSub?.land_asset_wrapper?.collateral_value?.value ?? "",
                        description: itemSub?.land_asset_wrapper?.description ?? "",
                        has_land_asset: "",
                        has_certificated_land_asset: "",
                        land_const_uuid: itemSub?.land_asset_wrapper?.land_const_uuid ?? ""
                      }

                    },

                    ctxd_gcn_qsh: { // cuc C
                      activeUuIdCtxdGcnQsh: itemSub?.certificated_land_asset_wrappers?.length > 0 ?  itemSub?.certificated_land_asset_wrappers[indexActiveCTXDGCNQSH]?.land_const_uuid : "",
                      ctxd_gcn_qsh_data: itemSub?.certificated_land_asset_wrappers?.map((ctxd_gcn, idxctxd_gcn) => {

                        const authInfo = item.sub_type[0]?.items[indexItem]?.ctxd_gcn_qsh?.ctxd_gcn_qsh_data[idxctxd_gcn]?.land_legal_information_owner
                        return  {
                        uuIdCtxdGcnQsh: ctxd_gcn?.land_const_uuid ?? "",
                        documents: mappingDocument(ctxd_gcn?.documents ?? []),
                        land_const_uuid: ctxd_gcn?.land_const_uuid ?? "",
                        land_legal_information_owner: {   // thong tin tab A 
                          active: authInfo.active ?? 0,
                          owner_type: ctxd_gcn?.owner_wrapper?.owner_type?.id,
                          owner: ctxd_gcn?.owner_wrapper?.owners?.map((ow) => {
                            const authActive = authInfo?.owner?.find(au => au.person_uuid === ow.person_uuid)?.active

                            return {
                            full_name: ow?.full_name,
                            authorized_persons: ow?.authorized_persons?.map(authPer => ({
                              documents: authPer.documents,
                              person_uuid: authPer?.person_uuid,
                              owner_relationship: authPer?.owner_relationship,
                              borrower_relationship: authPer?.borrower_relationship,
                              authorize_contract: authPer?.authorize_contract,
                              owner_auth_uuid: authPer?.owner_auth_uuid
                            })) ?? [],
                            has_authorize: ow?.has_authorize?.id ?? "N",
                            person_uuid: ow?.person_uuid,
                            active: authActive ?? 0,
                            owner_uuid: ow?.owner_uuid
                          }})
                        },

                        certificate_legal_info: { // thong tin tab B
                          activeUUIDCertificate: ctxd_gcn?.certificates[indexActiveCerCTXDGCNQSH]?.land_const_item_cert_uuid ?? "",
                          dataCertificate: ctxd_gcn?.certificates?.map(cer => ({
                            persons: cer?.persons?.map(per => ({
                              full_name: per?.full_name,
                              person_uuid: per?.person_uuid,
                              land_const_item_cert_item_uuid: per?.land_const_item_cert_item_uuid ?? ""
                            })),
                            documents: mappingDocument(cer.documents),
                            activeUUIDUserListLegal: generateUUID(),
                            activeUUIDCertificateL: cer?.land_const_item_cert_uuid ?? "",
                            typeUseLand: cer?.certificate_type?.id ?? "",
                            typeGCN: cer?.other_certificate_type ?? "",
                            numberGCNLegal: cer?.certificate_code ?? "",
                            numberGCN: cer?.certificate_no ?? "",
                            dateRange: cer?.issue_date ? (cer?.issue_date ?? 0) * 1000 : null,
                            dateLocation: cer?.place_of_issue ?? "",
                            land_const_item_cert_uuid: cer?.land_const_item_cert_uuid ?? ""
                          }))
                        },

                        ctxd_gcn_qsh_land_info: {  // thong tin tab C

                          dataCTXDLand: {
                            activeUUIDCTXDLand: generateUUID(),
                            asset_legal: ctxd_gcn?.land_asset?.asset_legal?.id ?? "",
                            legal_CTXD_other: ctxd_gcn?.land_asset?.other_asset_legal ?? null,
                            address: ctxd_gcn?.land_asset?.address ?? "",
                            provice: ctxd_gcn?.land_asset?.province?.id ?? "",
                            district: ctxd_gcn?.land_asset?.district?.id ?? "",
                            ward: ctxd_gcn?.land_asset?.ward?.id ?? "",
                            certificate_address: ctxd_gcn?.land_asset?.certificate_address ?? "",
                            certificate_province: ctxd_gcn?.land_asset?.certificate_province?.id ?? "",
                            certificate_district: ctxd_gcn?.land_asset?.certificate_district?.id ?? "",
                            certificate_ward: ctxd_gcn?.land_asset?.certificate_ward?.id ?? "",
                            activeUUIDtypeCTXD: 'activeUUIDtypeCTXD',
                            dataTypeCTXD: ctxd_gcn?.land_asset?.land_asset_types?.map((typed, indexType) => ({
                              activeTypeCTXD: indexType === 0 ? 'activeUUIDtypeCTXD' : generateUUID(),
                              land_asset_type: typed?.land_asset_type?.id ?? "",
                              land_asset_type_other: typed?.other_asset_type ?? "",
                              certificate_building_area: typed?.certificate_building_area?.toString() ?? "",
                              building_area: typed?.building_area?.value?.toString() ?? "",
                              certificate_cross_floor_area: typed?.certificate_cross_floor_area?.toString() ?? "",
                              cross_floor_area: typed?.cross_floor_area?.value?.toString() ?? "",
                              certificate_used_area: typed?.certificate_used_area?.toString() ?? "",
                              used_area: typed?.used_area?.value?.toString() ?? "",
                              ownership_duration: typed?.ownership_duration ?? "",
                              owner_form: typed?.owner_form ?? "",
                              certificate_structure: typed?.certificate_structure ?? "",
                              structure: typed?.structure ?? "",
                              certificate_rank: typed?.certificate_rank ?? "",
                              certificate_floors: typed?.certificate_floors ?? "",
                              floors: typed?.floors ?? "",
                              duration_of_use: typed?.duration_of_use ?? "",
                            })) ?? []
                          },
                          ctx_land_wrapper: {  // A. THÔNG TIN ĐỊNH GIÁ VÀ THẨM ĐỊNH TÀI SẢN  CHECK LAI UX 
                            from_credit_extension: ctxd_gcn?.from_credit_extension?.id ?? "",
                            is_exploited: ctxd_gcn?.is_exploited?.id ?? "N",
                            credit_extension_secured: ctxd_gcn?.credit_extension_secured?.id ?? "",
                            non_business_area: ctxd_gcn?.non_business_area?.value?.toString() ?? null,
                            max_percentage: ctxd_gcn?.max_percentage?.value?.toString() ?? null,
                            value_of_land: ctxd_gcn?.collateral_value?.value ?? "",
                            description: ctxd_gcn?.description ?? "",
                            has_land_asset: "",
                            has_certificated_land_asset: "",
                          },
                          activeCTXDLand: generateUUID(),
                        }

                      }}) ?? []
                    },
                    price_cert_asset_uuid: itemSub.price_cert_asset_uuid ?? "",

                  }
                  
                }) as unknown as ISubItems[]
              };
              // result.uuidItemsActive = _.get(_.first(result.items), 'activeUUID', '');
              
              return result;
            }) as ISubtype[],
            price_cert_uuid: action.payload?.price_cert_uuid
          } as ILOANNormalCollateralData;

        }
        return { ...item }
      })
      state.storage.collateral_v2.originData = state.storage.collateral_v2.data

    },
    prepare(
      payload: ICollateralFormGet,
      meta: string
    ) {
      return { payload, meta };
    },
  },


  setMachineLocation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILOANNormalCollateralData, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d = {
              ...d,
              province: action.payload.province,
              district: action.payload.district,
              ward: action.payload.ward,
            };
          }
          return { ...d };
        }
      );
    },
    prepare(
      payload: Pick<
        ILOANNormalCollateralData,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
      }
    ) {
      return { payload, meta };
    },
  },
  setLandLegalInfomationOwner: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuid: string;
          uuidActive: string;
          key: keyof ILandLegalInformationOwner;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                const activeUUIDType = i.uuidItemsActive;
                i.items?.map((iq) => {
                  let typeLand = iq.type_land;
                  if (iq.activeUUID === activeUUIDType) {
                    /**
                     *  Check theo loai => Type Land
                     */
                    if (typeLand === ETypeLandName.LAND) {
                      return (iq.land.land_legal_information_owner = {
                        ...iq.land.land_legal_information_owner,
                        [action.meta.key]: action.payload,
                      });
                    } else if (typeLand === ETypeLandName.CTXD_GCN) {
                      let uuidCTXD_GCN_Active =
                        iq.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                      iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data?.map((cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidCTXD_GCN_Active) {
                          return (cgqd.land_legal_information_owner = {
                            ...cgqd.land_legal_information_owner,
                            [action.meta.key]: action.payload,
                          });
                        }
                        return { ...cgqd };
                      });
                    }
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuid: string;
        uuidActive: string;
        key: keyof ILandLegalInformationOwner;
      }
    ) {
      return { payload, meta };
    },
  },

  /**
   * @todo Thông tin pháp lý thông tin đât
   * @param uuIdData
   * @param uuIdSubtype
   * @param key
   *
   */
  setLandInformationAsset: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null | string[] | number[],
        string,
        {
          uuIdData: string;
          uuIdSubtype: string;
          key: keyof ILegalLandInformatioAsset;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuIdData) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuIdSubtype) {
                const activeUUIDType = i.uuidItemsActive;
                i.items?.map((iq) => {
                  if (iq.activeUUID === activeUUIDType) {
                    iq.land.land_legal_infomation_asset = {
                      ...iq.land.land_legal_infomation_asset,
                      [action.meta.key]: action.payload,
                    };
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null | string[] | number[],
      meta: {
        uuIdData: string;
        uuIdSubtype: string;
        key: keyof ILegalLandInformatioAsset;
      }
    ) {
      return { payload, meta };
    },
  },

  setLandInformationAssetDataLocation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILegalLandInformatioAsset, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type = d.sub_type.map((st) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              let uuidItem = st.uuidItemsActive;
              st.items = st.items.map((i) => {
                if (i.activeUUID === uuidItem) {
                  i.land.land_legal_infomation_asset = {
                    ...i.land.land_legal_infomation_asset,
                    province: action.payload.province,
                    district: action.payload.district,
                    ward: action.payload.ward,
                  };
                }
                return { ...i };
              });
            }
            return { ...st };
          });
        }

        return { ...d };
      });
    },
    prepare(
      payload: Pick<
        ILegalLandInformatioAsset,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
        uuidSubType: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setLandInformationAssetDataLocationCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILegalLandInformatioAsset, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type = d.sub_type.map((st) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              let uuidItem = st.uuidItemsActive;
              st.items = st.items.map((i) => {
                if (i.activeUUID === uuidItem) {
                  i.land.land_legal_infomation_asset = {
                    ...i.land.land_legal_infomation_asset,
                    certificate_province: action.payload.province,
                    certificate_district: action.payload.district,
                    certificate_ward: action.payload.ward,
                  };
                }
                return { ...i };
              });
            }
            return { ...st };
          });
        }

        return { ...d };
      });
    },
    prepare(
      payload: Pick<
        ILegalLandInformatioAsset,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
        uuidSubType: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addCollaretalCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const activeCer = `${PREFIX_LOCAL}${generateUUID()}`;
                  if (type_land === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.activeUUIDCertificate = activeCer;
                    it.land.certificate_legal_info.dataCertificate.push({
                      ...generateEmptyDataCollateralCertificateUseListLegal(),
                      activeUUIDCertificateL: activeCer,
                    });
                  } else {
                    let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                        cgqd.certificate_legal_info.activeUUIDCertificate = activeCer;
                        cgqd.certificate_legal_info.dataCertificate.push({
                          ...generateEmptyDataCollateralCertificateUseListLegal(),
                          activeUUIDCertificateL: activeCer,
                        });
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let typeLand = it.type_land;

                if (it.activeUUID === action.meta.uuidActiveitems) {
                  if (typeLand === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.activeUUIDCertificate =
                      action.payload;
                  } else {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        cgqd.certificate_legal_info.activeUUIDCertificate =
                          action.payload;
                      }
                      return { ...cgqd };
                    });
                  }
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addCollaretalCertificateUserListLegal: {
    // use list legal
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  let typeLand = it.type_land;
                  const activeUseListLegal = generateUUID();

                  if (typeLand === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.dataCertificate.map((cre) => {
                      if (cre.activeUUIDCertificateL === action.meta.uuidActiveCer) {
                        cre.activeUUIDUserListLegal = activeUseListLegal;

                      }
                      return { ...cre };
                    });
                  } else {
                    let cdqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cdqd) => {
                      if (cdqd.uuIdCtxdGcnQsh === cdqd_active) {
                        cdqd.certificate_legal_info.dataCertificate.map((cre) => {
                          if (cre.activeUUIDCertificateL === action.meta.uuidActiveCer) {
                            cre.activeUUIDUserListLegal = activeUseListLegal;
                            // cre.userListDataLegal.push({
                            //   // ...generateEmptyDataCollateralCertificateUseListLegalDetails(),
                            //   // activeUUIDUserListLegal: activeUseListLegal,
                            // });
                          }
                          return { ...cre };
                        }
                        );
                      }
                      return { ...cdqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  /**
   * Remove user legal
   * Action paylod -> uuid user active
   * 
   */
  removeCollaretalCertificateUserListLegal: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.dataCertificate.map((cre) => {
                      if (cre.activeUUIDCertificateL === action.meta.uuidActiveCer) {
                        // cre.userListDataLegal = cre.userListDataLegal.filter(u =>
                        //   // u.activeUUIDUserListLegal !== action.payload
                        // );
                        cre.activeUUIDUserListLegal = ""
                      }
                      return { ...cre };
                    });
                  } else {
                    let cdqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cdqd) => {
                      if (cdqd.uuIdCtxdGcnQsh === cdqd_active) {
                        cdqd.certificate_legal_info.dataCertificate.map(cre => {
                          if (cre.activeUUIDCertificateL === action.meta.uuidActiveCer) {
                            // cre.userListDataLegal = cre.userListDataLegal.filter(u =>
                            //   // u.activeUUIDUserListLegal !== action.payload
                            // );
                            cre.activeUUIDUserListLegal = ""
                          }
                          return { ...cre }
                        })
                      }
                      return { ...cdqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  removeCollaretalCertificate: {
    reducer(state: Draft<ILOANNormalState>, action: PayloadAction<string, string,
      {
        type: string;
        uuidActiveData: string,
        uuidActiveSubtype: string,
        uuidActiveitems: string,
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        land_const_uuid: string;
        land_const_item_cert_uuid: string;
        land_cert_uuid: string;
      }>) {
      state.storage.collateral_v2.data.map(col => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map(i => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map(it => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  let typeLand = it.type_land;
                  // delete market certificates
                  if (typeLand === "MARK") {
                    it.market.maket_certificates = it.market.maket_certificates.filter(mar => mar.uuid_maket_certificate !== action.payload)
                    it.maketActiveUuid = it.market.maket_certificates[it.market.maket_certificates.length - 1]?.uuid_maket_certificate ?? ""
                  }
                  if (typeLand === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.dataCertificate = it.land.certificate_legal_info.dataCertificate.filter(cre => cre.activeUUIDCertificateL !== action.payload)
                    const indexLength = it.land.certificate_legal_info.dataCertificate.length - 1
                    it.land.certificate_legal_info.activeUUIDCertificate = it.land.certificate_legal_info.dataCertificate[indexLength]?.activeUUIDCertificateL ?? "";
                  }
                  else {
                    let cdqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(cdqd => {
                      if (cdqd.uuIdCtxdGcnQsh === cdqd_active) {
                        cdqd.certificate_legal_info.dataCertificate = cdqd.certificate_legal_info.dataCertificate.filter(cre => cre.activeUUIDCertificateL !== action.payload)
                        const indexLength = cdqd.certificate_legal_info.dataCertificate.length - 1
                        cdqd.certificate_legal_info.activeUUIDCertificate = cdqd.certificate_legal_info.dataCertificate[indexLength]?.activeUUIDCertificateL ?? "";
                      }
                      return { ...cdqd }
                    })
                  }

                }
                return { ...it }
              })
            }
            return { ...i }
          })
        }
        return { ...col }
      })
    },
    prepare(
      payload: string,
      meta: {
        type: string;
        uuidActiveData: string,
        uuidActiveSubtype: string,
        uuidActiveitems: string,
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        land_const_uuid: string;
        land_const_item_cert_uuid: string;
        land_cert_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeCollaretalCertificateMarket: {
    reducer(state: Draft<ILOANNormalState>, action: PayloadAction<string, string,
      {
        uuidActiveData: string,
        uuidActiveSubtype: string,
        uuidActiveitems: string,
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        market_cert_uuid: string;
      }>) {
      state.storage.collateral_v2.data.map(col => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map(i => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map(it => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  // delete market certificates
                  it.market.maket_certificates = it.market.maket_certificates.filter(mar => mar.uuid_maket_certificate !== action.payload)
                  it.maketActiveUuid = it.market.maket_certificates[it.market.maket_certificates.length - 1]?.uuid_maket_certificate ?? ""
                }
                return { ...it }
              })
            }
            return { ...i }
          })
        }
        return { ...col }
      })
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string,
        uuidActiveSubtype: string,
        uuidActiveitems: string,
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        market_cert_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateUserListLegal: {  // use list legal onchange
    reducer(state: Draft<ILOANNormalState>, action: PayloadAction<string, string,
      {
        uuidActiveData: string,
        uuidActiveSubtype: string,
        uuidActiveitems: string,
        uuidActiveCer: string,
      }>) {
      state.storage.collateral_v2.data.map(col => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let typeLand = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  if (typeLand === ETypeLandName.LAND) {
                    it.land.certificate_legal_info.dataCertificate.map((cre) => {
                      if (
                        cre.activeUUIDCertificateL === action.meta.uuidActiveCer
                      ) {
                        cre.activeUUIDUserListLegal = action.payload;
                      }
                      return { ...cre };
                    });
                  } else {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        cgqd.certificate_legal_info.dataCertificate.map(
                          (cre) => {
                            if (
                              cre.activeUUIDCertificateL ===
                              action.meta.uuidActiveCer
                            ) {
                              cre.activeUUIDUserListLegal = action.payload;
                            }
                            return { ...cre };
                          }
                        );
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateUserListLegalData: {
    // use list legal onchangeData
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
          uuidActiceUseList: string;
          key: keyof ICertificateLegalInfoDataUserList;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  let typeLand = it.type_land;
                  if (it.activeUUID === action.meta.uuidActiveitems) {
                    if (typeLand === ETypeLandName.LAND) {
                      it.land.certificate_legal_info.dataCertificate = it.land.certificate_legal_info.dataCertificate?.map(dd => {

                        if (dd.activeUUIDCertificateL === action.meta.uuidActiveCer) {
                          return {
                            ...dd,
                            [action.meta.key]: action.payload
                          }
                        }
                        return { ...dd }
                      });
                    } else {
                      let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {

                        if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                          const _activeUUIDCertificate = cgqd.certificate_legal_info.activeUUIDCertificate;
                          cgqd.certificate_legal_info.dataCertificate = cgqd.certificate_legal_info.dataCertificate.map(dc => {
                            if (dc.activeUUIDCertificateL === _activeUUIDCertificate) {
                              return {
                                ...dc,
                                [action.meta.key]: action.payload
                              }
                            }
                            return { ...dc }
                          })
                        }
                        return { ...cgqd };
                      });
                    }
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
        uuidActiceUseList: string;
        key: keyof ICertificateLegalInfoDataUserList;
      }
    ) {
      return { payload, meta };
    },
  },

  setActiveUuidLandInformationAssetType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuIdData: string;
          uuIdSubtype: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuIdData) {
          col.sub_type.map((i) => {
            let uuidItemActive = i.uuidItemsActive;

            if (i.uuidActiveSubtype === action.meta.uuIdSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === uuidItemActive) {
                  it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes =
                    action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuIdData: string;
        uuIdSubtype: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setLandInformationAssetType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuIdData: string;
          uuIdSubtype: string;
          key: keyof ILandAssetType;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuIdData) {
            col.sub_type = col.sub_type.map((i) => {
              let uuidItemActive = i.uuidItemsActive;

              if (i.uuidActiveSubtype === action.meta.uuIdSubtype) {
                i.items = i.items?.map((it) => {
                  if (it.activeUUID === uuidItemActive) {
                    let uuidCertificateUsePurposes =
                      it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes;

                    it.land.land_legal_infomation_asset.land_asset_types = it.land.land_legal_infomation_asset.land_asset_types.map(
                      (at) => {
                        if (at.activeUUIDCertificateUsePurposes === uuidCertificateUsePurposes) {
                          return {
                            ...at,
                            [action.meta.key]: action.payload,
                          };
                        }
                        return { ...at };
                      }
                    );
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuIdData: string;
        uuIdSubtype: string;
        key: keyof ILandAssetType;
      }
    ) {
      return { payload, meta };
    },
  },

  // Thông tin đẩt
  addLandInformationAsset: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuidData: string; uuidSubType: string }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              const activeUUID = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === activeUUID) {
                  let uuId = generateUUID();
                  it.land.land_legal_infomation_asset.land_asset_types.push({
                    ...generateEmptyDatacertificateUsePurposes(),
                    activeUUIDCertificateUsePurposes: uuId,
                  });
                  it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes = uuId;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },

  onChangeLandInformationAsset: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { uuidData: string; uuidSubType: string }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              const activeUUID = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === activeUUID) {
                  it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes = action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },

  /**
   * Action set data field info land asset
   * Action payload -> field names
   * 
   */
  setLandInformationAssetData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        { uuidData: string; uuidSubType: string, key: keyof ICertificateUsePurposes }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              const activeUUID = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === activeUUID) {
                  it.land.land_legal_infomation_asset.land_asset_types = it.land.land_legal_infomation_asset.land_asset_types.map(
                    (latd) => {
                      let uuidActiveLandAssetType = it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes;
                      if (latd.activeUUIDCertificateUsePurposes === uuidActiveLandAssetType) {
                        return {
                          ...latd,
                          [action.meta.key]: action.payload,
                        }
                      }
                      return { ...latd }
                    }
                  );
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidData: string;
        uuidSubType: string;
        key: keyof ICertificateUsePurposes
      }) {
      return { payload, meta };
    },
  },

  /**
   *  Action remove land asset
   *  action payload -> uuid active land asset
   * 
   */
  removeLandInformationAsset: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { 
          uuidData: string; 
          uuidSubType: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          re_land_used_uuid: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              const activeUUID = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === activeUUID) {

                  it.land.land_legal_infomation_asset.land_asset_types = it.land.land_legal_infomation_asset.land_asset_types.filter(lat =>
                    lat.activeUUIDCertificateUsePurposes !== action.payload
                  )
                  const index = it.land.land_legal_infomation_asset.land_asset_types.length - 1
                  it.land.land_legal_infomation_asset.activeUUIDCertificateUsePurposes = it.land.land_legal_infomation_asset.land_asset_types[index]?.activeUUIDCertificateUsePurposes ?? "";
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { 
      uuidData: string; 
      uuidSubType: string;
      price_cert_uuid: string; 
      price_cert_asset_uuid: string;
      re_land_used_uuid: string;
    }) {
      return { payload, meta };
    },
  },

  //////////CTXD trên đất ///////////
  addLandCTXD: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  const activeLandCTXD = generateUUID();
                  const typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.activeCTXDLand = activeLandCTXD;
                    it.ctxd_land.dataCTXDLand.push({
                      ...generateEmptyDataCollateralSubTypeLandCTXD(),
                      activeUUIDCTXDLand: activeLandCTXD,
                    });
                  }

                  if (typeLand === ETypeLandName.CTXD_GCN) {
                    // let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    // it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                    //   if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                    //     cgqd.ctxd_gcn_qsh_land_info.activeCTXDLand = activeLandCTXD;
                    //     cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.push({
                    //       ...generateEmptyDataCollateralSubTypeLandCTXD(),
                    //       activeUUIDCTXDLand: activeLandCTXD,
                    //     });
                    //   }
                    //   return { ...cgqd };
                    // });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: { uuidData: string; uuidSubType: string; uuidItems: string }
    ) {
      return { payload, meta };
    },
  },

  removeLandCTXD: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          land_const_item_uuid : string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {

              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  const typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.filter(
                      (cl) => cl.activeUUIDCTXDLand !== action.payload
                    )
                    const index = it.ctxd_land.dataCTXDLand.length - 1
                    it.ctxd_land.activeCTXDLand = it.ctxd_land.dataCTXDLand[index]?.activeUUIDCTXDLand ?? "";
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: { 
        uuidData: string; 
        uuidSubType: string; 
        uuidItems: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        land_const_item_uuid : string;
       }
    ) {
      return { payload, meta };
    },
  },

  onChangeHorizonListLandCTXD: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.activeCTXDLand = action.payload;
                  }

                  if (typeLand === ETypeLandName.CTXD_GCN) {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        cgqd.ctxd_gcn_qsh_land_info.activeCTXDLand = action.payload;
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: { uuidData: string; uuidSubType: string; uuidItems: string }
    ) {
      return { payload, meta };
    },
  },

  setDataLandCTXD: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        IValueOnChangeCollateral,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
          key: keyof ICTXDLandData;
          isSetAssetLegal?: boolean
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map((col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            return {
              ...col,
              sub_type : col.sub_type.map((i) => {
                if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                  return {
                    ...i,
                    items: i.items.map((it) => {
                      if (it.activeUUID === action.meta.uuidItems) {
                        let typeLand = it.type_land;
                        if (typeLand === ETypeLandName.CTXD_LAND) {
                          return {
                            ...it,
                            ctxd_land : {
                              ...it.ctxd_land,
                              dataCTXDLand: it.ctxd_land.dataCTXDLand.map((ct) => {
                                if (ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand) {
                                  if(!!action.meta.isSetAssetLegal && action.payload !== "ACCEPTOWN"){
                                    return {
                                      ...ct,
                                      [action.meta.key]: action.payload,
                                      certificate_address: "",
                                      certificate_district: "",
                                      certificate_province: "",
                                      certificate_ward: "",
                                    };
                                  }
                                  else {
                                    return {
                                      ...ct,
                                      [action.meta.key]: action.payload,
                                    };
                                  }
                                }
                                else return { ...ct };
                                
                              })
                            }
                          }
                          
                        }
    
                        if (typeLand === ETypeLandName.CTXD_GCN) {
                          let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                          return {
                            ...it,
                            ctxd_gcn_qsh: {
                              ...it.ctxd_gcn_qsh,
                              ctxd_gcn_qsh_data: it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                                  if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                                    return {
                                      ...cgqd,
                                      ctxd_gcn_qsh_land_info: {
                                        ...cgqd.ctxd_gcn_qsh_land_info,
                                        dataCTXDLand: {
                                          ...cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand,
                                          [action.meta.key]: action.payload,
                                        }
                                      }
                                    }
                                  }
                                  else return { ...cgqd };
                                }
                              )
                            }
                          }
                        }
                      }
                      return { ...it };
                    })
                  }
                }
                else return { ...i };
              })
            }
          }
          else return { ...col };
        }
      );
    },
    prepare(
      payload: IValueOnChangeCollateral,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
        key: keyof ICTXDLandData;
        isSetAssetLegal?: boolean
      }
    ) {
      return { payload, meta };
    },
  },

  setDataLandCTXDLocation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILOANNormalStorageAddress, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidItems) {
                    let typeLand = it.type_land;

                    if (typeLand === ETypeLandName.CTXD_LAND) {
                      it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.map(
                        (ct) => {
                          if (
                            ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand
                          ) {
                            ct = {
                              ...ct,
                              provice: action.payload.province,
                              district: action.payload.district,
                              ward: action.payload.ward,
                            };
                            return {
                              ...ct,
                            };
                          }
                          return { ...ct };
                        }
                      );
                    }

                    if (typeLand === ETypeLandName.CTXD_GCN) {
                      let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.provice = action.payload.province;
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.district = action.payload.district;
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.ward = action.payload.ward;
                        }
                        return { ...cgqd };
                      });
                    }
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: Pick<
        ILOANNormalStorageAddress,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setDataLandCTXDLocationCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILOANNormalStorageAddress, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidItems) {
                    let typeLand = it.type_land;

                    if (typeLand === ETypeLandName.CTXD_LAND) {
                      it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.map(
                        (ct) => {
                          if (
                            ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand
                          ) {
                            ct = {
                              ...ct,
                              certificate_province: action.payload.province,
                              certificate_district: action.payload.district,
                              certificate_ward: action.payload.ward,
                            };
                            return {
                              ...ct,
                            };
                          }
                          return { ...ct };
                        }
                      );
                    }

                    if (typeLand === ETypeLandName.CTXD_GCN) {
                      let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.certificate_province = action.payload.province;
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.certificate_district = action.payload.district;
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.certificate_ward = action.payload.ward;
                        }
                        return { ...cgqd };
                      });
                    }
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: Pick<
        ILOANNormalStorageAddress,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },

  //////////Loại CTXD trên đất ///////////
  addLandCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.dataCTXDLand.map((ct) => {
                      if (ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand) {
                        const activeTypeCTXD = generateUUID();
                        ct.activeUUIDtypeCTXD = activeTypeCTXD;
                        ct.dataTypeCTXD.push({
                          ...generateEmptyDataCollateralSubTypeCTXD(),
                          activeTypeCTXD: activeTypeCTXD,
                        });
                      }
                      return { ...ct };
                    });
                  }

                  if (typeLand === ETypeLandName.CTXD_GCN) {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        const activeTypeCTXD = generateUUID();

                        cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.activeUUIDtypeCTXD = activeTypeCTXD;
                        cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD.push({
                          ...generateEmptyDataCollateralSubTypeCTXD(),
                          activeTypeCTXD: activeTypeCTXD,
                        });
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },

  /**
   *  Xóa loại công trình xây dựng
   * action payload => uuid delete
   * 
   */
  removeLandCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          type: string;
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
          price_cert_uuid: string;
          price_cert_asset_uuid: string;
          land_const_uuid: string;
          land_const_item_uuid: string;
          land_const_item_detail_uuid: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.dataCTXDLand.map((ct) => {
                      if (ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand) {
                        ct.dataTypeCTXD = ct.dataTypeCTXD.filter(t => t.activeTypeCTXD !== action.payload)
                        ct.activeUUIDtypeCTXD = ct.dataTypeCTXD[ct.dataTypeCTXD.length - 1]?.activeTypeCTXD ?? ""
                      }
                      return { ...ct };
                    });
                  }

                  if (typeLand === ETypeLandName.CTXD_GCN) {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD = cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD.filter(
                          t => t.activeTypeCTXD !== action.payload
                        )
                        const index = cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD.length - 1
                        cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.activeUUIDtypeCTXD = cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD[index]?.activeTypeCTXD ?? ""
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        type: string;
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        land_const_uuid: string;
        land_const_item_uuid: string;
        land_const_item_detail_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },

  onChangeHorizonListLandCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.dataCTXDLand.map((ctType) => {
                      if (
                        ctType.activeUUIDCTXDLand === action.meta.uuidCTXDLand
                      ) {
                        ctType.activeUUIDtypeCTXD = action.payload;
                      }
                      return { ...ctType };
                    });
                  }

                  // if (typeLand === ETypeLandName.CTXD_GCN) {
                  //   let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                  //   it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                  //     if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                  //       cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.map((ctType) => {
                  //         if (
                  //           ctType.activeUUIDCTXDLand ===
                  //           action.meta.uuidCTXDLand
                  //         ) {
                  //           ctType.activeUUIDtypeCTXD = action.payload;
                  //         }
                  //         return { ...ctType };
                  //       });
                  //     }
                  //     return { ...cgqd };
                  //   });
                  // }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },

  onChangeLandCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              i.items.map((it) => {
                if (it.activeUUID === action.meta.uuidItems) {
                  let typeLand = it.type_land;

                  if (typeLand === ETypeLandName.CTXD_LAND) {
                    it.ctxd_land.dataCTXDLand.map((ct) => {
                      ct.activeUUIDtypeCTXD = action.payload;
                      return { ...ct };
                    });
                  }

                  if (typeLand === ETypeLandName.CTXD_GCN) {
                    let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                      if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                        cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.activeUUIDtypeCTXD = action.payload;
                      }
                      return { ...cgqd };
                    });
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setDataLandCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
          uuidCTXDLandType: string;
          key: keyof ITypeCTXD;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidItems) {
                    let typeLand = it.type_land;

                    if (typeLand === ETypeLandName.CTXD_LAND) {
                      it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.map(
                        (ct) => {
                          if (
                            ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand
                          ) {
                            ct.dataTypeCTXD = ct.dataTypeCTXD.map((ctType) => {
                              if (
                                ctType.activeTypeCTXD ===
                                action.meta.uuidCTXDLandType
                              ) {
                                return {
                                  ...ctType,
                                  [action.meta.key]: action.payload,
                                };
                              }
                              return { ...ctType };
                            });
                          }
                          return { ...ct };
                        }
                      );
                    }

                    if (typeLand === ETypeLandName.CTXD_GCN) {
                      let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                          cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD = cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand.dataTypeCTXD.map(
                            (ctType) => {
                              if (
                                ctType.activeTypeCTXD ===
                                action.meta.uuidCTXDLandType
                              ) {
                                return {
                                  ...ctType,
                                  [action.meta.key]: action.payload,
                                };
                              }
                              return { ...ctType };
                            }
                          );
                        }
                        return { ...cgqd };
                      });
                    }
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
        uuidCTXDLandType: string;
        key: keyof ITypeCTXD;
      }
    ) {
      return { payload, meta };
    },
  },
  clearFieldCTXDType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          uuidCTXDLand: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidItems) {
                    let typeLand = it.type_land;

                    if (typeLand === ETypeLandName.CTXD_LAND) {
                      it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.map(
                        (ct) => {
                          if ( ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand ) {
                            return {
                              ...ct,
                              certificate_address: "",
                              certificate_province: "",
                              certificate_district: "",
                              certificate_ward: ""
                            }
                          }
                          return { ...ct };
                        }
                      );
                    }

                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },
  /**
   * @todo: CTXD có GCN QSH riêng
   */
  addLandGcnQsh: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              let uuidItemActive = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === uuidItemActive) {
                  const uuId = generateUUID();
                  it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh = uuId;
                  it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.push({
                    ...generateEmptyDataCtxdGcnQshData(),
                    uuIdCtxdGcnQsh: uuId,
                  });
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },

  deleteLandGcnQsh: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          land_const_uuid: string;
          isNoti: boolean
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              let uuidItemActive = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === uuidItemActive) {
                  it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.filter(
                    gq => gq.uuIdCtxdGcnQsh !== action.payload
                  );
                  it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh =
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data[it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.length - 1]?.uuIdCtxdGcnQsh ?? "";
                  if(it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.length === 0){
                    it.land.land_wrapper.has_certificated_land_asset = "N"
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { 
      uuidData: string; 
      uuidSubType: string;
      price_cert_uuid: string; 
      price_cert_asset_uuid: string;
      land_const_uuid: string ;
      isNoti: boolean
    }) {
      return { payload, meta };
    },
  },

  setUuidActiveLandGcnQsh: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidSubType) {
              let uuidItemActive = i.uuidItemsActive;
              i.items.map((it) => {
                if (it.activeUUID === uuidItemActive) {
                  it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh = action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(payload: string, meta: { uuidData: string; uuidSubType: string }) {
      return { payload, meta };
    },
  },



  setMaketInfomation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        { uuIdData: string; uuIdSubtype: string; key: keyof IMaketInfo }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuIdData) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuIdSubtype) {
                const activeUUIDType = i.uuidItemsActive;
                i.items?.map((iq) => {
                  if (iq.activeUUID === activeUUIDType) {
                    return (iq.market.maket_info = {
                      ...iq.market.maket_info,
                      [action.meta.key]: action.payload,
                    });
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: { uuIdData: string; uuIdSubtype: string; key: keyof IMaketInfo }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateMaket: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.maketActiveUuid = action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addCollaretalCertificateMaket: {
    // use list legal
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                const activeUseListLegal = `${PREFIX_LOCAL}${generateUUID()}`;
                it.maketActiveUuid = activeUseListLegal;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.market.maket_certificates.push({
                    ...generateEmptyDataMaketCertificates(),
                    uuid_maket_certificate: activeUseListLegal,
                  });
                }


                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addCollaretalCertificateMaketPersionListLegal: {
    // use list legal
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {

                  it.market.maket_certificates.map((cre) => {
                    if (
                      cre.uuid_maket_certificate === action.meta.uuidActiveCer
                    ) {
                      // cre.uuid_active_persion = activePersion;
                      // cre.persons.push({
                      //   ...generateEmptyDataMaketCertificatePersion(),
                      //   person_uuid: activePersion,
                      // });
                    }
                    return { ...cre };
                  });
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateMaketPersionListLegal: {
    // use list legal onchange
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  // it.maket.maket_certificates.map((mc) => {
                  //   if (
                  //     mc.uuid_maket_certificate === action.meta.uuidActiveCer
                  //   ) {
                  //     mc.uuid_active_persion = action.payload;
                  //   }
                  //   return { ...mc };
                  // });
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  // Maket persion
  setOnChangeCollaretalCertificateMaketPersionListLegalData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
          key: keyof IMaketCertificates;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  if (it.activeUUID === action.meta.uuidActiveitems) {
                    it.market.maket_certificates = it.market.maket_certificates?.map((mc) => {
                      if (
                        mc.uuid_maket_certificate === action.meta.uuidActiveCer
                      ) {
                        return {
                          ...mc,
                          [action.meta.key]: action.payload
                        }
                      }
                      return { ...mc };
                    });
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
        key: keyof IMaketCertificates;
      }
    ) {
      return { payload, meta };
    },
  },

  /// deparment
  addCollaretalCertificateDepartment: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data?.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                const activeUseListLegal = `${PREFIX_LOCAL}${generateUUID()}`;
                it.departmentActiveUUID = activeUseListLegal;

                // it.department.department_certificate_legal.push({
                //   ...generateEmptyDataDepartmentCertificates(),
                //   uuid_certificate_legal: activeUseListLegal,
                // });
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department.department_certificate_legal.push({
                    ...generateEmptyDataDepartmentCertificates(),
                    uuid_certificate_legal: activeUseListLegal

                  })
                }



                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  removeCollaretalCertificateDepartment: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          price_cert_uuid: string;
          price_cert_asset_uuid: string;
          apart_owner_cert_uuid: string;
        }
      >
    ) {
      state.storage.collateral_v2.data?.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department.department_certificate_legal = it.department.department_certificate_legal.filter(
                    dcl => dcl.uuid_certificate_legal !== action.payload
                  )
                  it.departmentActiveUUID = it.department.department_certificate_legal[it.department.department_certificate_legal.length - 1]?.uuid_certificate_legal ?? "";
                }
                return { ...it };

              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        apart_owner_cert_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setCollaretalCertificateDepartment: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          key: keyof IDepartment
        }
      >
    ) {
      state.storage.collateral_v2.data?.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department = {
                    ...it.department,
                    [action.meta.key]: action.payload
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        key: keyof IDepartment
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateDepartment: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.departmentActiveUUID = action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  addCollaretalCertificateDepartmentPersonListLegal: {
    // use list legal
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {

                  it.department.department_certificate_legal.map((cre) => {
                    if (
                      cre.uuid_certificate_legal === action.meta.uuidActiveCer
                    ) {
                      // cre.uuid_active_person = activePersion;
                      // cre.persons.push({
                      //   ...generateEmptyDataDepartmentCertificatesUserList(),
                      //   uuid_active_person: activePersion,
                      // });
                    }
                    return { ...cre };
                  });
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificatDepartmentListLegal: {
    // use list legal onchange
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department.department_certificate_legal.map((mc) => {
                    if (
                      mc.uuid_certificate_legal === action.meta.uuidActiveCer
                    ) {
                      // mc.uuid_active_person = action.payload;
                    }
                    return { ...mc };
                  });
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalCertificateDepartmentListLegalData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
          key: keyof IPersonCertificateLegal;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  if (it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_certificate_legal = it.department.department_certificate_legal.map(
                      de => {
                        if (de.uuid_certificate_legal === action.meta.uuidActiveCer) {
                          return {
                            ...de,
                            [action.meta.key]: action.payload
                          }
                        }
                        return { ...de }
                      }
                    );
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
        key: keyof IPersonCertificateLegal;
      }
    ) {
      return { payload, meta };
    },
  },
  clearOnChangeCollaretalDeparmentHasCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  if (it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_certificate_legal = it.department.department_certificate_legal.map(
                      de => {
                        // clear thong tin giay chung nhan khi thay doi  tinh trang phap ly
                        if(it.department.has_certificate === 'N'){
                          return {
                            ...de,
                            other_certificate_type: "",
                            other_certificate_type_other: "",
                            certificate_code: "",
                            certificate_no: null,
                            issue_date: null,
                            place_of_issue: ''
                          }
                        }else{
                          return {
                            ...de,
                            contract_number_type: '',
                            contract_number: '',
                            contract_date: null,
                          }
                        }
                      }
                    );
                    if(it.department.has_certificate === 'N'){
                      it.department.department_info_land.use_purposes = []
                    }
                    
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },
  //// type BDS thong tin can ho
  addCollaretalDepartmentInfo: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                const activeUUID = generateUUID();
                it.departmentInfoActiveUUID = activeUUID;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department.department_info.push({
                    ...generateEmptyDataDepartmentInfo(),
                    departmentInfoActiveUUID: activeUUID,
                  });
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },


  removeCollaretalDepartmentInfo: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          apart_room_uuid: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {

                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.department.department_info = it.department.department_info.filter(
                    di => di.departmentInfoActiveUUID !== action.payload
                  )
                  it.departmentInfoActiveUUID = it.department.department_info[it.department.department_info.length - 1]?.departmentInfoActiveUUID ?? "";
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        apart_room_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalDepartmentInfo: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                if (
                  it.departmentInfoActiveUUID === action.meta.uuidActiveitems
                ) {
                  it.departmentInfoActiveUUID = action.payload;
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setOnChangeCollaretalDepartmentInfoData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveDepartment: string;
          key: keyof IDepartmentInfo;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_info = it.department.department_info?.map((de) => {
                      if (
                        de.departmentInfoActiveUUID ===
                        action.meta.uuidActiveDepartment
                      ) {
                        return {
                          ...de,
                          [action.meta.key]: action.payload,
                        };
                      }
                      return { ...de };
                    });
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveDepartment: string;
        key: keyof IDepartmentInfo;
      }
    ) {
      return { payload, meta };
    },
  },

  /// thong tin dat du an

  setLandInformationDataLocation: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<IDepartmentInfoLand, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type = d.sub_type.map((st) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              st.items = st.items.map((i) => {
                if (i.activeUUID === action.meta.uuidItem) {
                  i.department.department_info_land = {
                    ...i.department.department_info_land,
                    province: action.payload.province,
                    district: action.payload.district,
                    ward: action.payload.ward,
                  };
                }
                return { ...i };
              });
            }
            return { ...st };
          });
        }

        return { ...d };
      });
    },
    prepare(
      payload: Pick<IDepartmentInfoLand, "province" | "district" | "ward">,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
      }
    ) {
      return { payload, meta };
    },
  },
  setLandInformationDataLocationCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        Pick<ILegalLandInformatioAsset, "province" | "district" | "ward">,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((d) => {
        if (d.uuidActiveData === action.meta.uuidData) {
          d.sub_type = d.sub_type.map((st) => {
            if (st.uuidActiveSubtype === action.meta.uuidSubType) {
              st.items = st.items.map((i) => {
                if (i.activeUUID === action.meta.uuidItem) {
                  i.department.department_info_land = {
                    ...i.department.department_info_land,
                    certificate_province: action.payload.province,
                    certificate_district: action.payload.district,
                    certificate_ward: action.payload.ward,
                  };
                }
                return { ...i };
              });
            }
            return { ...st };
          });
        }

        return { ...d };
      });
    },
    prepare(
      payload: Pick<
        ILegalLandInformatioAsset,
        "province" | "district" | "ward"
      >,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
      }
    ) {
      return { payload, meta };
    },
  },
  setLandInformationData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null | string[] | number[],
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
          key: keyof IDepartmentInfoLand;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d.sub_type = d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubType) {
                st.items = st.items.map((i) => {
                  if (i.activeUUID === action.meta.uuidItem) {
                    i.department.department_info_land = {
                      ...i.department.department_info_land,
                      [action.meta.key]: action.payload,
                    };
                  }
                  return { ...i };
                });
              }
              return { ...st };
            });
          }
          return { ...d };
        }
      );
    },
    prepare(
      payload: string | number | null | string[] | number[],
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
        key: keyof IDepartmentInfoLand;
      }
    ) {
      return { payload, meta };
    },
  },

  addLandCertificateInfo: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d.sub_type = d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubType) {
                st.items = st.items.map((i) => {
                  // const hasCertificate = i.department.has_certificate;
                  if (i.activeUUID === action.meta.uuidItem) {
                    // if (hasCertificate === 'N') {
                    //   i.department.department_info_land.certificate_use_purposes = []
                    // } else {
                      const activeUUID = generateUUID();
                      i.activeUUIDCertificateUsePurposes = activeUUID;
                      i.department.department_info_land.certificate_use_purposes.push({
                        ...generateEmptyDatacertificateUsePurposes(),
                        activeUUIDCertificateUsePurposes: activeUUID,
                      });
                    // }
                  }
                  return { ...i };
                });
              }
              return { ...st };
            });
          }
          return { ...d };
        }
      );
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
      }
    ) {
      return { payload, meta };
    },
  },
  callRemoveLandCertificateInfoUsePurposes:{
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        { 
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
          price_cert_uuid: string;
          price_cert_asset_uuid: string;
          apart_land_used_uuid: string;
        }
      >
    ) {

    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
        price_cert_uuid: string;
        price_cert_asset_uuid: string;
        apart_land_used_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeLandCertificateInfoUsePurposes: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d.sub_type = d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubType) {
                st.items = st.items.map((i) => {
                  if (i.activeUUID === action.meta.uuidItem) {

                    i.department.department_info_land.certificate_use_purposes = i.department.department_info_land.certificate_use_purposes.filter(
                      cp => cp.activeUUIDCertificateUsePurposes !== action.payload
                    )
                    const index = i.department.department_info_land.certificate_use_purposes.length - 1
                    i.activeUUIDCertificateUsePurposes = i.department.department_info_land.certificate_use_purposes[index]?.activeUUIDCertificateUsePurposes ?? "";
                  }
                  return { ...i };
                });
              }
              return { ...st };
            });
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
      }
    ) {
      return { payload, meta };
    },
  },

  onLandCertificateInfo: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d.sub_type = d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubType) {
                st.items = st.items.map((i) => {
                  if (i.activeUUID === action.meta.uuidItem) {
                    i.activeUUIDCertificateUsePurposes = action.payload;
                  }
                  return { ...i };
                });
              }
              return { ...st };
            });
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: string,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setLandCertificateInfoData: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItem: string;
          uuidCertificate: string;
          key: keyof ICertificateUsePurposes
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (d) => {
          if (d.uuidActiveData === action.meta.uuidData) {
            d.sub_type = d.sub_type.map((st) => {
              if (st.uuidActiveSubtype === action.meta.uuidSubType) {
                st.items = st.items.map((i) => {
                  if (i.activeUUID === action.meta.uuidItem) {
                    i.department.department_info_land.certificate_use_purposes = i.department.department_info_land.certificate_use_purposes.map(cre => {
                      if (cre.activeUUIDCertificateUsePurposes === action.meta.uuidCertificate) {
                        return {
                          ...cre,
                          [action.meta.key]: action.payload
                        }
                      }
                      return { ...cre }
                    })
                  }
                  return { ...i };
                });
              }
              return { ...st };
            });
          }

          return { ...d };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItem: string;
        uuidCertificate: string;
        key: keyof ICertificateUsePurposes
      }
    ) {
      return { payload, meta };
    },
  },


  autoFillREST(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2 = autoFillBDS as ILOANNormalCollateralV2State
  },
  autoFillRESTDepartment(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2.data.push({ ...autofillMARKETTTT(), uuidActiveData: generateUUID() } as unknown as ILOANNormalCollateralData)
  },
  autoFillRESTLANDDDDDDDDDDDD(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2.data.push({ ...autofillLANDDDDDD(), uuidActiveData: generateUUID() } as unknown as ILOANNormalCollateralData)
  },
  autoFillRESTMARKETTTTTTTTTT(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2.data.push({ ...autofillDEPARTTTTTTTT(), uuidActiveData: generateUUID() } as unknown as ILOANNormalCollateralData)
  },



  ///////////////////////

  /// chọn đối tượng sở hữu tài sản
  setCollaretalOwnerType: {

    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          objdataLegal: ILandOwner[];
          typeOwner: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;

              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  let type = it.market.market_owner.owner_type;
                  it.market.market_owner.owner_type = action.payload;

                  if (type !== action.meta.typeOwner) {
                    it.market.market_owner.owner = action.meta.objdataLegal;

                  } else {
                    const listOwner = it.market.market_owner.owner;
                    const newOwner = action.meta.objdataLegal?.filter(({ person_uuid: id1 }) => !listOwner?.some(({ person_uuid: id2 }) => id2 === id1))
                    it.market.market_owner.owner = listOwner.concat(newOwner);
                  }
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    let type = it.land.land_legal_information_owner.owner_type;
                    it.land.land_legal_information_owner.owner_type = action.payload;

                    if (type !== action.meta.typeOwner) {
                      it.land.land_legal_information_owner.owner = action.meta.objdataLegal;

                    } else {
                      const listOwner = it.land.land_legal_information_owner.owner;
                      const newOwner = action.meta.objdataLegal?.filter(({ person_uuid: id1 }) => !listOwner?.some(({ person_uuid: id2 }) => id2 === id1))
                      it.land.land_legal_information_owner.owner = listOwner.concat(newOwner);
                    }
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          const type = cgqd.land_legal_information_owner.owner_type;
                          cgqd.land_legal_information_owner.owner_type = action.payload;

                          if (type !== action.meta.typeOwner) {
                            cgqd.land_legal_information_owner.owner = action.meta.objdataLegal
                          } else {
                            const listOwner = cgqd.land_legal_information_owner.owner;
                            const newOwner = action.meta.objdataLegal?.filter(({ person_uuid: id1 }) => !listOwner?.some(({ person_uuid: id2 }) => id2 === id1))
                            cgqd.land_legal_information_owner.owner = listOwner.concat(newOwner);
                          }

                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  const type = it.department.department_owner.owner_type;
                  it.department.department_owner.owner_type = action.payload;

                  if (type !== action.meta.typeOwner) {
                    it.department.department_owner.owner = action.meta.objdataLegal
                  } else {
                    const listOwner = it.department.department_owner.owner;
                    const newOwner = action.meta.objdataLegal?.filter(({ person_uuid: id1 }) => !listOwner?.some(({ person_uuid: id2 }) => id2 === id1))
                    it.department.department_owner.owner = listOwner.concat(newOwner);
                  }
                } else if (it.activeUUID === activeUUIDType) {
                  const type = it.owner_wrapper.owner_type;
                  it.owner_wrapper.owner_type = action.payload;

                  if (type !== action.meta.typeOwner) {
                    it.owner_wrapper.owner = action.meta.objdataLegal
                  } else {
                    const listOwner = it.owner_wrapper.owner;
                    const newOwner = action.meta.objdataLegal?.filter(({ person_uuid: id1 }) => !listOwner?.some(({ person_uuid: id2 }) => id2 === id1))
                    it.owner_wrapper.owner = listOwner.concat(newOwner);
                  }
                }

                return { ...it };
              });

            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        objdataLegal: ILandOwner[];
        typeOwner: string;
      }
    ) {
      return { payload, meta };
    },
  },


  /// set người sở hữu đang được chọn
  setOnChangeCollaretalActiveOwner: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;
              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  it.market.market_owner.active = Number(action.payload);
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    it.land.land_legal_information_owner.active = Number(action.payload);
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          cgqd.land_legal_information_owner.active = Number(action.payload);
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  it.department.department_owner.active = Number(action.payload);
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  ///// set có văn bản ủy quyền hay không

  setCollaretalInfoOwnerHasAuthorize: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;
              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  if (it.market.market_owner.owner.length > 0) {
                    const activeMarket = it.market.market_owner.active;
                    it.market.market_owner.owner[activeMarket].has_authorize = action.payload
                    if (it.market.market_owner.owner[activeMarket].has_authorize === 'N') {
                      it.market.market_owner.owner[activeMarket].authorized_persons = []
                    }
                  }
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    if (it.land.land_legal_information_owner.owner.length > 0) {
                      const activeLand = it.land.land_legal_information_owner.active;
                      it.land.land_legal_information_owner.owner[activeLand].has_authorize = action.payload
                      if (it.land.land_legal_information_owner.owner[activeLand].has_authorize === 'N') {
                        it.land.land_legal_information_owner.owner[activeLand].authorized_persons = []
                      }
                    }
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          if (cgqd.land_legal_information_owner.owner.length > 0) {
                            const activeLand = cgqd.land_legal_information_owner.active;
                            cgqd.land_legal_information_owner.owner[activeLand].has_authorize = action.payload
                            if (cgqd.land_legal_information_owner.owner[activeLand].has_authorize === 'N') {
                              cgqd.land_legal_information_owner.owner[activeLand].authorized_persons = []
                            }
                          }
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  if (it.department.department_owner.owner.length > 0) {
                    const activeAPA = it.department.department_owner.active;
                    it.department.department_owner.owner[activeAPA].has_authorize = action.payload
                    if (it.department.department_owner.owner[activeAPA].has_authorize === 'N') {
                      it.department.department_owner.owner[activeAPA].authorized_persons = []
                    }
                  }

                }

                return { ...it };
              });

            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },
  //// thêm người đồng sở hữu type đồng sở hữu
  setCollaretalOwnerModal: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          objdataLegal: ILandOwner[]

        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;

              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  it.market.market_owner.owner = action.meta.objdataLegal;
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    it.land.land_legal_information_owner.owner = action.meta.objdataLegal;

                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          cgqd.land_legal_information_owner.owner = action.meta.objdataLegal
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  it.department.department_owner.owner = action.meta.objdataLegal

                } else if (it.activeUUID === activeUUIDType) {
                  it.owner_wrapper.owner = action.meta.objdataLegal
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        objdataLegal: ILandOwner[]
      }
    ) {
      return { payload, meta };
    },
  },

  /////// thêm người được ủy quyền
  setCollaretalHasAuthor: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          objdataLegal: IInfoAuthorize[];
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;

              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  const active = it.market.market_owner.active;
                  it.market.market_owner.owner[active].authorized_persons = action.meta.objdataLegal;
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    const active = it.land.land_legal_information_owner.active;
                    it.land.land_legal_information_owner.owner[active].authorized_persons = action.meta.objdataLegal;
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          const active = cgqd.land_legal_information_owner.active
                          cgqd.land_legal_information_owner.owner[active].authorized_persons = action.meta.objdataLegal;

                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  const active = it.department.department_owner.active;
                  it.department.department_owner.owner[active].authorized_persons = action.meta.objdataLegal;

                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        objdataLegal: IInfoAuthorize[];
      }
    ) {
      return { payload, meta };
    },
  },

  /// người ủy quyền được chọn
  setOnChangeCollaretalActiveHasAuthor: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;
              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  const active = it.market.market_owner.active;
                  it.market.market_owner.owner[active].active = Number(action.payload);
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    const active = it.land.land_legal_information_owner.active;
                    it.land.land_legal_information_owner.owner[active].active = Number(action.payload);
                  }
                  /**
                 * Map theo loại đất
                 * TH: CTXD có GCN QSH riêng
                 */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          const active = cgqd.land_legal_information_owner.active
                          cgqd.land_legal_information_owner.owner[active].active = Number(action.payload);
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo chung cư
                   */
                  const active = it.department.department_owner.active;
                  it.department.department_owner.owner[active].active = Number(action.payload);
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },
  /// set dữ liệu của người đượ ủy quyền
  setOnChangeCollaretalDetailHasAuthor: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | ILOANNormalDocument[] | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          key: keyof IInfoAuthorize;
        }
      >
    ) {
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;
              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  const active = it.market.market_owner.active;
                  const activeHasAuthor = it.market.market_owner.owner[active].active;
                  it.market.market_owner.owner[active].authorized_persons[activeHasAuthor] = {
                    ...it.market.market_owner.owner[active].authorized_persons[activeHasAuthor],
                    [action.meta.key]: action.payload

                  };

                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    const active = it.land.land_legal_information_owner.active;
                    const activeHasAuthor = it.land.land_legal_information_owner.owner[active].active;
                    it.land.land_legal_information_owner.owner[active].authorized_persons[activeHasAuthor] = {
                      ...it.land.land_legal_information_owner.owner[active].authorized_persons[activeHasAuthor],
                      [action.meta.key]: action.payload

                    };
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          const active = cgqd.land_legal_information_owner.active
                          const activeHasAuthor = cgqd.land_legal_information_owner.owner[active].active;
                          cgqd.land_legal_information_owner.owner[active].authorized_persons[activeHasAuthor] = {
                            ...cgqd.land_legal_information_owner.owner[active].authorized_persons[activeHasAuthor],
                            [action.meta.key]: action.payload

                          };
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  const active = it.department.department_owner.active;
                  const activeHasAuthor = it.department.department_owner.owner[active].active;
                  it.department.department_owner.owner[active].authorized_persons[activeHasAuthor] = {
                    ...it.department.department_owner.owner[active].authorized_persons[activeHasAuthor],
                    [action.meta.key]: action.payload

                  };

                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string | number | ILOANNormalDocument[] |null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        key: keyof IInfoAuthorize;
      }
    ) {
      return { payload, meta };
    },
  },

  setCollaretalCertificatePersion: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
          listPerson: IPerson[]
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  if (i.id === ESubtypeRest.MARK && it.activeUUID === action.meta.uuidActiveitems) {
                    it.market.maket_certificates = it.market.maket_certificates?.map((mc) => {
                      if (
                        mc.uuid_maket_certificate === action.meta.uuidActiveCer
                      ) {
                        return {
                          ...mc,
                          'persons': action.meta.listPerson
                        }
                      }
                      return { ...mc };
                    });
                  } else if (i.id === ESubtypeRest.LAND && it.activeUUID === action.meta.uuidActiveitems) {
                    if (it.type_land === ETypeLandName.LAND) {
                      it.land.certificate_legal_info.dataCertificate = it.land.certificate_legal_info.dataCertificate?.map((mc) => {
                        if (
                          mc.activeUUIDCertificateL === action.meta.uuidActiveCer
                        ) {
                          return {
                            ...mc,
                            'persons': action.meta.listPerson
                          }
                        }
                        return { ...mc };
                      });
                    }
                    if (it.type_land === ETypeLandName.CTXD_GCN) {
                      let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                        (cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                            cgqd.certificate_legal_info.dataCertificate = cgqd.certificate_legal_info.dataCertificate?.map((mc) => {
                              if (
                                mc.activeUUIDCertificateL === action.meta.uuidActiveCer
                              ) {
                                return {
                                  ...mc,
                                  'persons': action.meta.listPerson
                                }
                              }
                              return { ...mc };
                            });
                          }
                          return { ...cgqd };
                        }
                      );

                    }
                  } else if (i.id === ESubtypeRest.APPA && it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_certificate_legal = it.department.department_certificate_legal?.map((mc) => {
                      if (
                        mc.uuid_certificate_legal === action.meta.uuidActiveCer
                      ) {
                        return {
                          ...mc,
                          'persons': action.meta.listPerson
                        }
                      }
                      return { ...mc };
                    });
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
        listPerson: IPerson[];
      }
    ) {
      return { payload, meta };
    },
  },
  /// xóa người sở hữu
  removeCollaretalCertificatePerson: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  if (i.id === ESubtypeRest.MARK && it.activeUUID === action.meta.uuidActiveitems) {
                    it.market.maket_certificates = it.market.maket_certificates?.map((mc) => {
                      if (
                        mc.uuid_maket_certificate === action.meta.uuidActiveCer
                      ) {
                        mc.persons = mc.persons.filter(p => p.person_uuid !== action.payload)
                      }
                      return { ...mc };
                    });
                  } else if (i.id === ESubtypeRest.LAND && it.activeUUID === action.meta.uuidActiveitems) {
                    if (it.type_land === ETypeLandName.LAND) {
                      it.land.certificate_legal_info.dataCertificate = it.land.certificate_legal_info.dataCertificate?.map((mc) => {
                        if (
                          mc.activeUUIDCertificateL === action.meta.uuidActiveCer
                        ) {
                          mc.persons = mc.persons.filter(p => p.person_uuid !== action.payload)

                        }
                        return { ...mc };
                      });
                    }
                    if (it.type_land === ETypeLandName.CTXD_GCN) {
                      let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                        (cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                            cgqd.certificate_legal_info.dataCertificate = cgqd.certificate_legal_info.dataCertificate?.map((mc) => {
                              if (
                                mc.activeUUIDCertificateL === action.meta.uuidActiveCer
                              ) {
                                mc.persons = mc.persons.filter(p => p.person_uuid !== action.payload)
                              }
                              return { ...mc };
                            });
                          }
                          return { ...cgqd };
                        }
                      );

                    }
                  } else if (i.id === ESubtypeRest.APPA && it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_certificate_legal = it.department.department_certificate_legal?.map((mc) => {
                      if (
                        mc.uuid_certificate_legal === action.meta.uuidActiveCer
                      ) {
                        mc.persons = mc.persons.filter(p => p.person_uuid !== action.payload)
                      }
                      return { ...mc };
                    });
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
      }
    ) {
      return { payload, meta };
    },
  },
  /// xóa người sở hữu gcn
  callRemoveCollaretalCertificatePerson: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          type: string;
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCer: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          apart_owner_cert_uuid: string;
          apart_owner_cert_item_uuid: string;
          market_cert_uuid: string;
          market_cert_item_uuid: string;
          land_const_uuid: string;
          land_const_item_cert_uuid: string;
          land_const_item_cert_item_uuid: string;
          land_cert_uuid: string;
          re_land_cert_item_uuid: string;
        }
      >
    ) {
    },
    prepare(
      payload: string | number | null,
      meta: {
        type: string;
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCer: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        apart_owner_cert_uuid: string;
        apart_owner_cert_item_uuid: string;
        market_cert_uuid: string;
        market_cert_item_uuid: string;
        land_const_uuid: string;
        land_const_item_cert_uuid: string;
        land_const_item_cert_item_uuid: string;
        land_cert_uuid: string;
        re_land_cert_item_uuid: string;
      }
    ) {
      return { payload, meta };
    },
  },

  setCheckboxTransportType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          dataMeta: CheckboxValue[];
          parentId?: string;
          otherFlag: string;
          otherValue?: string | null;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type = col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items = i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  // add list parent
                  if (!action.meta.parentId) {
                    // for(let i in action.meta.dataMeta){
                    console.log(i);
                    if (action.meta.dataMeta[0]?.checked === true) {
                      it.legal_document_types?.push({
                        ...generateEmptyTypeTransportLegal(),
                        id: action.meta.dataMeta[0].value
                      })
                    }
                    else {
                      it.legal_document_types = it.legal_document_types?.filter(i => i.id !== action.meta.dataMeta.find(j => j.value === i.id)?.value)
                    }
                    // }
                  }
                  // add list child
                  else {
                    let parent = it.legal_document_types?.find(i => i.id === action.meta.parentId)

                    it.legal_document_types = it.legal_document_types?.map(doc => {
                      if (doc.id === action.meta.parentId) {
                        if (action.meta.dataMeta[0].checked === true) {

                          parent?.documents?.push({
                            ...generateEmptyTypeTransportLegalDocumentsChild(action.meta.otherFlag),
                            id: action.meta.dataMeta[0]?.value
                          })
                        } else {
                          doc.documents = doc.documents?.filter(i => i.id !== action.meta.dataMeta[0].value)
                          if (action.meta.otherFlag === "Y" && action.meta.dataMeta[0].checked === false) {
                            doc.other_document = ""
                          }
                        }
                      }
                      return { ...doc }
                    })

                  }
                }
                return { ...it };
              });

            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        dataMeta: CheckboxValue[];
        parentId: string | undefined;
        otherFlag: string;
        otherValue?: string | null;
      }
    ) {
      return { payload, meta };
    },
  },
  setCheckboxTransportTypeInput: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          parentId?: string;
          otherFlag: string;
          otherValue?: string | null;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type = col.sub_type?.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items = i.items?.map((it) => {
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  it.legal_document_types = it.legal_document_types?.map(docs => {
                    if (docs.id === action.meta.parentId) {
                      docs.other_document = action.meta.otherValue
                    }
                    return { ...docs }
                  })
                }
                return { ...it };
              });

            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        parentId: string | undefined;
        otherFlag: string;
        otherValue?: string | null;
      }
    ) {
      return { payload, meta };
    },
  },

  /// 
  autoFillRESTMarket(state: ILOANNormalState, action: PayloadAction<string>) {
    state.storage.collateral_v2 = autofillRestMarket as ILOANNormalCollateralV2State
  },
  setCollateralValidate(state: Draft<ILOANNormalState>, action: PayloadAction<ILOANNormalStorageLoanValidate>) {
    state.storage.collateral_v2.validate = action.payload;
    if(action.payload.position){
      state.storage.collateral_v2.uuidActiveData = action.payload.position.toString()
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(data => {
        if(data.uuidActiveData === action.payload?.data){
          return {
            ...data,
            is_collapse_type: true
          }
        }
        else return {
          ...data,
          is_collapse_type: false
        }
      })
    }
  },
  clearStorageCollateral(state: Draft<ILOANNormalState>) {
    state.storage.collateral_v2.data = []
    state.storage.collateral_v2.activeType = "ALL"
    state.storage.collateral_v2.carousel = []
    state.storage.collateral_v2.uuidActiveData = ""
  },

  setCollaretalCertificate: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number | null,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
        }
      >
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidActiveData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                i.items = i.items?.map((it) => {
                  it.description = ''
                  it.typeCollateral =''
                  it.license =''
                  it.status =''
                  // it.status_flag?.id = '';
                  it.count = null
                  it.year = null
                  it.branch =''
                  it.model = ''
                  it.production =''
                  it.CLCL = ''
                  it.number_register = ''
                  it.quantity =null
                  it.brand =''
                  it.issuer = ''

                  if (i.id === ESubtypeRest.MARK && it.activeUUID === action.meta.uuidActiveitems) {
                    it.market.maket_certificates = [];                  
                  } 
                  else if (i.id === ESubtypeRest.LAND && it.activeUUID === action.meta.uuidActiveitems) {
                    if (it.type_land === ETypeLandName.LAND) {
                      it.land.certificate_legal_info.dataCertificate = []                     
                    }
                    if (it.type_land === ETypeLandName.CTXD_GCN) {
                      let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                        (cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                            cgqd.certificate_legal_info.dataCertificate = []
                          }
                          return { ...cgqd };
                        }
                      );

                    }
                  } 
                  else if (i.id === ESubtypeRest.APPA && it.activeUUID === action.meta.uuidActiveitems) {
                    it.department.department_certificate_legal = []
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: string | number | null,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
      }
    ) {
      return { payload, meta };
    },
  },

  /// xóa người được ủy quyền
  removeCollaretalOwner: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          type: string;
          uuidActiveData: string;
          uuidActiveSubtype: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          land_const_uuid: string;
          owner_uuid: string; 
        }
      >
    ) {
      
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;

              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  it.market.market_owner.owner = it.market.market_owner.owner?.filter(a => a.person_uuid !== action.payload);
                  it.market.market_owner.active = 0
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    it.land.land_legal_information_owner.owner = it.land.land_legal_information_owner.owner?.filter(a => a.person_uuid !== action.payload);
                    it.land.land_legal_information_owner.active = 0;
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          cgqd.land_legal_information_owner.owner = cgqd.land_legal_information_owner.owner?.filter(a => a.person_uuid !== action.payload);
                          cgqd.land_legal_information_owner.active = 0;
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  it.department.department_owner.owner = it.department.department_owner.owner?.filter(a => a.person_uuid !== action.payload);
                  it.department.department_owner.active = 0;

                }else{
                  it.owner_wrapper.owner = it.owner_wrapper.owner.filter(i=>i.person_uuid !== action.payload)
                  it.owner_wrapper.active = 0
                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        type: string;
        uuidActiveData: string;
        uuidActiveSubtype: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        land_const_uuid: string;
        owner_uuid: string; 
      }
    ) {
      return { payload, meta };
    },
  },
  /// xóa người được ủy quyền
  removeCollaretalHasAuthor: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          type: string;
          uuidActiveData: string;
          uuidActiveSubtype: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          land_const_uuid: string;
          owner_uuid: string; 
          owner_auth_uuid : string;
        }
      >
    ) {
      
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              const activeUUIDType = i.uuidItemsActive;

              i.items?.map((it) => {
                if (i.id === ESubtypeRest.MARK && it.activeUUID === activeUUIDType) {
                  const active = it.market.market_owner.active;
                  it.market.market_owner.owner[active].authorized_persons = it.market.market_owner.owner[active].authorized_persons?.filter(a => a.person_uuid !== action.payload);
                  it.market.market_owner.owner[active].active = 0;
                }
                else if (i.id === ESubtypeRest.LAND && it.activeUUID === activeUUIDType) {
                  /**
                   * Map theo loại đất
                   * TH: Đất
                   */
                  if (it.type_land === ETypeLandName.LAND) {
                    const active = it.land.land_legal_information_owner.active;
                    it.land.land_legal_information_owner.owner[active].authorized_persons = it.land.land_legal_information_owner.owner[active].authorized_persons?.filter(a => a.person_uuid !== action.payload);
                    it.land.land_legal_information_owner.owner[active].active = 0;
                  }
                  /**
                   * Map theo loại đất
                   * TH: CTXD có GCN QSH riêng
                   */
                  if (it.type_land === ETypeLandName.CTXD_GCN) {
                    let uuidActiveCtxdGcnQsh = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                    it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                      (cgqd) => {
                        if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                          const active = cgqd.land_legal_information_owner.active
                          cgqd.land_legal_information_owner.owner[active].authorized_persons = cgqd.land_legal_information_owner.owner[active].authorized_persons?.filter(a => a.person_uuid !== action.payload);
                          cgqd.land_legal_information_owner.owner[active].active = 0;
                        }
                        return { ...cgqd };
                      }
                    );
                  }
                } else if (i.id === ESubtypeRest.APPA && it.activeUUID === activeUUIDType) {
                  const active = it.department.department_owner.active;
                  it.department.department_owner.owner[active].authorized_persons = it.department.department_owner.owner[active].authorized_persons?.filter(a => a.person_uuid !== action.payload);
                  it.department.department_owner.owner[active].active = 0;

                }

                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        type: string;
        uuidActiveData: string;
        uuidActiveSubtype: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        land_const_uuid: string;
        owner_uuid: string; 
        owner_auth_uuid : string;
      }
    ) {
      return { payload, meta };
    },
  },
  fetchListCollateral(state: Draft<ILOANNormalState>) {
    state.storage.collateral_v2.fetching = true;
  },
  fetchListCollateralDone(state: Draft<ILOANNormalState>) {
    state.storage.collateral_v2.fetching = false;
    state.storage.collateral_v2.fetched = true;
  },

  /**
   *  ******************* handle document here **********************
   */
  addNewCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string,
        }
      >
    ) {

      const addDocLand = (cer: ICertificateLegalInfoData) => {
        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
          const empty: ILOANNormalDocument = {
            child_files: [],
            document_id: '',
            document_name: '',
            uuid: `${PREFIX_LOCAL}${generateUUID()}`
          }
          if (!cer.documents) {
            cer.documents = [];
          }
          cer.documents.push(empty);
        }
        return { ...cer };
      }

      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          const empty: ILOANNormalDocument = {
                            child_files: [],
                            document_id: '',
                            document_name: '',
                            uuid: `${PREFIX_LOCAL}${generateUUID()}`
                          }
                          if (!cer.documents) {
                            cer.documents = [];
                          }
                          cer.documents.push(empty);
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          const empty: ILOANNormalDocument = {
                            child_files: [],
                            document_id: '',
                            document_name: '',
                            uuid: `${PREFIX_LOCAL}${generateUUID()}`
                          }
                          if (!cer.documents) {
                            cer.documents = [];
                          }
                          cer.documents.push(empty);
                        }
                      })
                      break;
                    }
                    default: {
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(addDocLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(addDocLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  addNewCollaretalCertificateFile: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string,
          uuidDoc: string;
        }
      >
    ) {
      const addFileToDoc = (doc: ILOANNormalDocument) => {
        if (doc.uuid === action.meta.uuidDoc) {
          if (!doc.child_files) {
            doc.child_files = [];
          }
          const empty: ILOANNormalFile = {
            uuid: `${PREFIX_LOCAL}${generateUUID()}`,
            content_type: null,
            created_at: null,
            created_by: "",
            created_by_name: "",
            updated_at: null,
            updated_by: "",
            updated_by_name: "",
            description: null,
            display_order: null,
            file_id: null,
            name: null,
            file_upload: null,
          };
          doc.child_files.push(empty)
        }
      }
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          cer.documents.map(addFileToDoc);
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents.map(addFileToDoc);
                        }
                      })
                      break;
                    }
                    default: {
                      const addFileLand = (cer: ICertificateLegalInfoData) => {
                        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
                          cer?.documents?.map(addFileToDoc);
                        }
                        return { ...cer };
                      }
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(addFileLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(addFileLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        uuidDoc: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  updateCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          uuidDoc: string;
          optionData: { label: string, value: string };
          itemDocType?: string;
        }
      >
    ) {
      const updateDoc = (doc: ILOANNormalDocument) => {
        if (doc.uuid === action.meta.uuidDoc) {
          doc.document_id = action.payload;
          doc.document_name = _.get(action, 'meta.optionData.label', '');
        }
      };
      // 
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateDoc)
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateDoc);
                        }
                      })
                      break;
                    }
                    default: {
                      const updateDocLand = (cer: ICertificateLegalInfoData) => {
                        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
                          cer?.documents?.map(updateDoc)
                        }
                        return { ...cer };
                      }
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(updateDocLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(updateDocLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string | number,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        uuidDoc: string;
        optionData: { label: string, value: string };
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  updateCollaretalCertificateFile: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        ILOANNormalFile[],
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          uuidDoc: string;
          itemDocType?: string;
        }
      >
    ) {
      const updateFile = (doc: ILOANNormalDocument) => {
        if (doc.uuid === action.meta.uuidDoc) {
          action?.payload?.map(payloadFile => {
            if (!doc?.child_files) return;
            const idx = doc.child_files?.findIndex(file => file.uuid === payloadFile.uuid);
            if (idx !== -1) doc.child_files[idx] = { ...payloadFile };
          });
        }
      }

      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateFile);
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateFile);
                        }
                      })
                      break;
                    }
                    default: {
                      const updateFileLand = (cer: ICertificateLegalInfoData) => {
                        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
                          cer?.documents?.map(updateFile)
                        }
                        return { ...cer };
                      }
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(updateFileLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(updateFileLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: ILOANNormalFile[],
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        uuidDoc: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  updateDescriptionCollaretalCertificateFile: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          uuidDoc: string;
          uuidFile: string;
          itemDocType?: string;
        }
      >
    ) {
      const updateDesc = (doc: ILOANNormalDocument) => {
        if (doc.uuid === action.meta.uuidDoc) {
          doc.child_files?.map(file => {
            if (file.uuid === action.meta.uuidFile) {
              file.description = action.payload;
            }
          })
        }
      };

      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateDesc);
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents.map(updateDesc);
                        }
                      })
                      break;
                    }
                    default: {
                      const updateDescLand = (cer: ICertificateLegalInfoData) => {
                        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
                          cer?.documents?.map(updateDesc)
                        }
                        return { ...cer };
                      }
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(updateDescLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(updateDescLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }
                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
        uuidDoc: string;
        uuidFile: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string;
        }
      >
    ) {
      const removeDoc = (cer: ICertificateLegalInfoData) => {
        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
          if (cer?.documents) {
            cer.documents = cer.documents.filter(doc => doc.uuid !== action.payload);
          }
        }
        return { ...cer };
      }
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          if (cer?.documents) {
                            cer.documents = cer.documents.filter(doc => doc.uuid !== action.payload);
                          }
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents = cer.documents.filter(doc => doc.uuid !== action.payload);
                        }
                      })
                      break;
                    }
                    default: {
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(removeDoc);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(removeDoc);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }

                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeAllCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string | number,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string;
        }
      >
    ) {
      const removeAll = (cer: ICertificateLegalInfoData) => {
        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
          if (cer?.documents) {
            cer.documents = [];
          }
        }
        return { ...cer };
      }
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          if (cer?.documents) {
                            cer.documents = [];
                          }
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents = [];
                        }
                      })
                      break;
                    }
                    default: {
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(removeAll);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(removeAll);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }

                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string | number,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeCollaretalCertificateFile: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          uuidDoc: string;
          itemDocType?: string;
        }
      >
    ) {
      const removeFile = (doc: ILOANNormalDocument) => {
        if (doc.uuid === action.meta.uuidDoc) {
          if (doc?.child_files) {
            doc.child_files = doc.child_files.filter(file => file.uuid !== action.payload);
          }
        }
      }
      state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          col.sub_type.map((i) => {
            if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
              i.items?.map((it) => {
                let type_land = it.type_land;
                if (it.activeUUID === action.meta.uuidActiveitems) {
                  const itemDocType = _.get(action, 'meta.itemDocType', '');
                  switch (itemDocType) {
                    case 'department': {
                      it.department.department_certificate_legal.map((cer) => {
                        if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                          if (cer?.documents) {
                            cer.documents.map(removeFile);
                          }
                        }
                      })
                      break;
                    }
                    case 'market': {
                      it.market.maket_certificates.map((cer) => {
                        if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                          cer.documents.map(removeFile);
                        }
                      })
                      break;
                    }
                    default: {
                      const removeFileLand = (cer: ICertificateLegalInfoData) => {
                        if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
                          if (cer?.documents) {
                            cer.documents.map(removeFile)
                          }
                        }
                        return { ...cer };
                      }
                      if (type_land === ETypeLandName.LAND) {
                        it.land.certificate_legal_info.dataCertificate.map(removeFileLand);
                      } else if (type_land === ETypeLandName.CTXD_GCN) {
                        let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                            cgqd.certificate_legal_info.dataCertificate.map(removeFileLand);
                          }
                          return { ...cgqd };
                        });
                      }
                    }
                  }

                }
                return { ...it };
              });
            }
            return { ...i };
          });
        }
        return { ...col };
      });
    },
    prepare(
      payload: string,
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        uuidDoc: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  saveCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        ILOANNormalDocument[],
        string,
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string;
        }
      >
    ) {
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  removeActionCollaretalDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        ITypeActionSaveDocumentCollateralMetaData
      >
    ) {
    },
    prepare(
      payload: string,
      meta: ITypeActionSaveDocumentCollateralMetaData
    ) {
      return { payload, meta };
    },
  },
  saveCollaretalDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeActionSaveDocumentCollateral
    ) {
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: ITypeActionSaveDocumentCollateralMetaData
    ) {
      return { payload, meta };
    },
  },
  mappingAfterUploadCollaretalType: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeMappingDocumentAction<{
        uuidActiveData: string
      }>
    ) {
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          return {
            ...col, 
            documents: action.payload
          }
        }
        return { ...col };
      });
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: {
        uuidActiveData: string
      }
    ) {
      return { payload, meta };
    },
  },

  mappingAfterUploadDataLandCTXD: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeMappingDocumentAction<{
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }>
    ) {
      // const mappingDocument = (docs: ILOANNormalDocument[]) => {
      //   return docs?.map((d) => ({
      //     ...d,
      //     child_files:
      //       d?.child_files?.map((f) => {
      //         const exist = action.payload.find(
      //           (a) => f?.uuid === a?.custom_keys?.local_uuid
      //         );
      //         if (exist) {
      //           return { ...exist };
      //         }
      //         return { ...f };
      //       }) ?? [],
      //   }))
      // }
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuidData) {
            col.sub_type = col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidSubType) {
                i.items = i.items.map((it) => {
                  if (it.activeUUID === action.meta.uuidItems) {
                    let typeLand = it.type_land;

                    if (typeLand === ETypeLandName.CTXD_LAND) {
                      it.ctxd_land.dataCTXDLand = it.ctxd_land.dataCTXDLand.map(
                        (ct) => {
                          if (
                            ct.activeUUIDCTXDLand === action.meta.uuidCTXDLand
                          ) {
                            return {
                              ...ct,
                              documents: action.payload
                              // [action.meta.key]: action.payload,
                            };
                          }
                          return { ...ct };
                        }
                      );
                    }

                    if (typeLand === ETypeLandName.CTXD_GCN) {
                      let cgqd_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;

                      it.ctxd_gcn_qsh.ctxd_gcn_qsh_data = it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map(
                        (cgqd) => {
                          if (cgqd.uuIdCtxdGcnQsh === cgqd_active) {
                            cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand = {
                              ...cgqd.ctxd_gcn_qsh_land_info.dataCTXDLand,
                              documents: []
                            }
                          }
                          return { ...cgqd };
                        }
                      );
                    }
                  }
                  return { ...it };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: {
        uuidData: string;
        uuidSubType: string;
        uuidItems: string;
        uuidCTXDLand: string;
      }
    ) {
      return { payload, meta };
    },
  },
  mappingAfterUploadDataLandCTXDGCN: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeMappingDocumentAction<{ uuid: string; uuidActive: string; }>
    ) {
      // const mappingDocument = (docs: ILOANNormalDocument[]) => {
      //   return docs?.map((d) => ({
      //     ...d,
      //     child_files:
      //       d?.child_files?.map((f) => {
      //         const exist = action.payload.find(
      //           (a) => f?.uuid === a?.custom_keys?.local_uuid
      //         );
      //         if (exist) {
      //           return { ...exist };
      //         }
      //         return { ...f };
      //       }) ?? [],
      //   }))
      // }
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                const activeUUIDType = i.uuidItemsActive;
                i.items = i.items?.map((iq) => {
                  if (i.id === ESubtypeRest.LAND && iq.activeUUID === activeUUIDType) {
                    if (iq.activeUUID === activeUUIDType) {
                      if (iq.type_land === ETypeLandName.CTXD_GCN) {
                        let uuidActiveCtxdGcnQsh =
                          iq.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                        iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data = iq.ctxd_gcn_qsh.ctxd_gcn_qsh_data?.map(
                          (cgqd) => {
                            if (cgqd.uuIdCtxdGcnQsh === uuidActiveCtxdGcnQsh) {
                              return { ...cgqd, documents: action.payload };
                            }
                            return { ...cgqd };
                          }
                        );
                      }

                    }
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: { uuid: string; uuidActive: string; }
    ) {
      return { payload, meta };
    },
  },
  mappingAfterUploadSubItemDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeMappingDocumentAction<{
        uuid: string; uuidActive: string;
      }>
    ) {
      // const mappingDocument = (docs: ILOANNormalDocument[]) => {
      //   return docs?.map((d) => ({
      //     ...d,
      //     child_files:
      //       d?.child_files?.map((f) => {
      //         const exist = action.payload.find(
      //           (a) => f?.uuid === a?.custom_keys?.local_uuid
      //         );
      //         if (exist) {
      //           return { ...exist };
      //         }
      //         return { ...f };
      //       }) ?? [],
      //   }))
      // }
      state.storage.collateral_v2.data = state.storage.collateral_v2.data?.map(
        (col) => {
          if (col.uuidActiveData === action.meta.uuid) {
            col.sub_type = col.sub_type?.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActive) {
                const activeUUIDType = i.uuidItemsActive;
                i.items = i.items?.map((iq) => {
                  if (iq.activeUUID === activeUUIDType) {
                    return {
                      ...iq,
                      documents: action.payload
                    };
                  }
                  return { ...iq };
                });
              }
              return { ...i };
            });
          }
          return { ...col };
        }
      );
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: {
        uuid: string; uuidActive: string;
      }
    ) {
      return { payload, meta };
    },
  },
  mappingAfterUploadCollaretalCertificateDocument: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: ITypeMappingDocumentAction<
        {
          uuidActiveData: string;
          uuidActiveSubtype: string;
          uuidActiveitems: string;
          uuidActiveCertificate: string;
          itemDocType?: string;
        }
      >
    ) {
      // const mappingCertificateDocument = (cer: ICertificateLegalInfoData) => {
      //   if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate) {
      //     return {
      //       ...cer,
      //       documents: action.payload
      //     }
      //   }
      //   return { ...cer };
      // };
      
      state.storage.collateral_v2.data = state.storage.collateral_v2.data.map((col) => {
        if (col.uuidActiveData === action.meta.uuidActiveData) {
          return {
            ...col,
            sub_type: col.sub_type.map((i) => {
              if (i.uuidActiveSubtype === action.meta.uuidActiveSubtype) {
                return {
                  ...i,
                  items: i.items?.map((it) => {
                    let type_land = it.type_land;
                    if (it.activeUUID === action.meta.uuidActiveitems) {
                      const itemDocType = _.get(action, 'meta.itemDocType', '');
                      switch (itemDocType) {
                        case 'department': {
                          return {
                            ...it,
                            department: {
                              ...it.department,
                              department_certificate_legal: it.department.department_certificate_legal.map(cer => {
                                if (cer.uuid_certificate_legal === action.meta.uuidActiveCertificate) {
                                  return {
                                    ...cer,
                                    documents: action.payload
                                  }
                              }
                               else return {...cer}
                            })
                          }
                        }}
                        case 'market': {
                          return {
                            ...it,
                            market: {
                              ...it.market,
                              maket_certificates:  it.market.maket_certificates.map(cer => {
                                if (cer.uuid_maket_certificate === action.meta.uuidActiveCertificate) {
                                  return {
                                    ...cer,
                                    documents: action.payload
                                  }}
                                  else return {...cer}
                                }
                              )
                            }
                          }
                        }
                        default: {
                          if (type_land === ETypeLandName.LAND) {
                            return {
                              ...it,
                              land: {
                                ...it.land,
                                certificate_legal_info: {
                                  ...it.land.certificate_legal_info,
                                  dataCertificate: it.land.certificate_legal_info.dataCertificate.map(cer => {
                                    if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate){
                                      return {
                                        ...cer,
                                        documents: action.payload
                                      }
                                    }
                                    else return {...cer}
                                  })
                                }
                              }
                            }
                            // it.land.certificate_legal_info.dataCertificate.map(mappingCertificateDocument);
                          } else if (type_land === ETypeLandName.CTXD_GCN) {
                            let cgqd_uuid_active = it.ctxd_gcn_qsh.activeUuIdCtxdGcnQsh;
                            return {
                              ...it,
                              ctxd_gcn_qsh: {
                                ...it.ctxd_gcn_qsh,
                                ctxd_gcn_qsh_data: it.ctxd_gcn_qsh.ctxd_gcn_qsh_data.map((cgqd) => {
                                  if (cgqd.uuIdCtxdGcnQsh === cgqd_uuid_active) {
                                    return {
                                      ...cgqd,
                                      certificate_legal_info :  {
                                        ...cgqd.certificate_legal_info,
                                        dataCertificate: cgqd.certificate_legal_info.dataCertificate.map(cer => {
                                          if (cer.activeUUIDCertificateL === action.meta.uuidActiveCertificate){
                                            return {
                                              ...cer,
                                              documents: action.payload
                                            }
                                          }
                                          else return {...cer}
                                        })
                                      }
                                    }
                                  }
                                  else return { ...cgqd };
                                })
                              }
                            }
                          }
                        }
                      }
                    
                    }
                    return { ...it };
                  })
                }
                
              }
              return { ...i };
            })
          }
        }
        return { ...col };
      });
    },
    prepare(
      payload: ILOANNormalDocument[],
      meta: {
        uuidActiveData: string;
        uuidActiveSubtype: string;
        uuidActiveitems: string;
        uuidActiveCertificate: string;
        itemDocType?: string;
      }
    ) {
      return { payload, meta };
    },
  },
  onChangeIgnoreData(state: Draft<ILOANNormalState>, action: PayloadAction<string>) {
    state.storage.collateral_v2.ignore = action.payload
  },
  deleteCollateralItem: {
    reducer(state: Draft<ILOANNormalState>, action: PayloadAction<string, string, {
      price_cert_uuid: string,
      price_cert_asset_uuid: string,
      uuidData?: string,
      uuidSubType?: string,
    }>) {
      if (state.storage.full.data) {

        state.storage.full.data.form.collateral_form.data.collaterals =
          state.storage.full.data.form.collateral_form.data.collaterals.map(listColl => {
            if (listColl.price_cert_uuid === action.meta.price_cert_uuid) {
              const result = listColl.sub_types[0].items.filter(i => i.price_cert_asset_uuid !== action.meta.price_cert_asset_uuid)
              return {
                ...listColl,
                sub_types: listColl.sub_types.map(sub => {
                  return {
                    items: result
                  }
                }) as ISubTypeGet[]
              }
            }
            return { ...listColl }
          })
      }
    },
    prepare(payload: string, meta: {
      price_cert_uuid: string,
      price_cert_asset_uuid: string,
      uuidData?: string,
      uuidSubType?: string,
    }) {
      return { payload, meta };
    }
  },
  removeLandAsset: {
    reducer(
      state: Draft<ILOANNormalState>,
      action: PayloadAction<
        string,
        string,
        {
          uuidData: string;
          uuidSubType: string;
          uuidItems: string;
          price_cert_uuid: string; 
          price_cert_asset_uuid: string;
          land_const_uuid : string;
        }
      >
    ) {
      // state.storage.collateral_v2.data.map((col) => {
      //   if (col.uuidActiveData === action.meta.uuidData) {
      //     col.sub_type.map((i) => {
      //       if (i.uuidActiveSubtype === action.meta.uuidSubType) {

      //         i.items.map((it) => {
      //           if (it.activeUUID === action.meta.uuidItems) {
      //             const typeLand = it.type_land;
                  
      //           }
      //           return { ...it };
      //         });
      //       }
      //       return { ...i };
      //     });
      //   }
      //   return { ...col };
      // });
    },
    prepare(
      payload: string,
      meta: { 
        uuidData: string; 
        uuidSubType: string; 
        uuidItems: string;
        price_cert_uuid: string; 
        price_cert_asset_uuid: string;
        land_const_uuid : string;
       }
    ) {
      return { payload, meta };
    },
  },
  setOriginData(state: Draft<ILOANNormalState>) {
    state.storage.collateral_v2.originData = state.storage.collateral_v2.data
  },
};
