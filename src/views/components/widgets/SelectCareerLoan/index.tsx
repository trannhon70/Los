...
export interface SelectCareerLoanProps {
  label?: ReactNode;
  required?: boolean;
  message?: string;
  value?: string;
...
const SelectCareerLoan: FC<SelectCareerLoanProps> = props => {
  ...
  const options = metadataConstant?.data[METADATA_CONSTANT.MA_NGANH_NGHE]?.map(c => ({ value: c.id, label: c.name })) ?? [];
...
export default SelectCareerLoan;