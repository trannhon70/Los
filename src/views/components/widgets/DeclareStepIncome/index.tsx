...
import { getCustomerTotal, getLOANNormalIncomeDeclareCompleted, getLOANNormalLegalData } from 'features/loan/normal/storage/income/selector';
...
const DeclareStepIncome: DeclareStepComponent = props => {
  ...
  const checkStatusPersonInLegal = (typeDeclare: string): { exist: boolean, completed: boolean } => {
    switch (typeDeclare) {
      case 'BORROWER': return { exist: !!(legalData.borrower?.basic_info?.uuid), completed: IncomeDeclareCompleted.borrower };
      case 'MARRIAGE': return { exist: !!(legalData.marriage?.basic_info?.uuid), completed: IncomeDeclareCompleted.marriage };
      case 'CO_BRW': return { exist: !!legalData.coBorrower && legalData.coBorrower?.some(item => !!item.basic_info.uuid), completed: IncomeDeclareCompleted.coBorrower };
      case 'CO_PAYER': return { exist: !!legalData.coPayer && legalData.coPayer?.some(item => !!item.basic_info.uuid), completed: IncomeDeclareCompleted.coPayer };
    ...
    return PersonalRep.filter(p => !~exclude.indexOf(p.id))
      .map((person, i) => {
        ...
        return {
          ...
          disabled:((ruleDisabled && TotalData[i] > 0) ? false :
           (TotalData[i] === 0 && ruleDisabled) ? true : 
           (TotalData[i] === 0 && !ruleDisabled) ? !exist :false),
