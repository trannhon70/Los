...
export interface IModalCheckConfirmProps {
  open: boolean;
  onClose?: () => void;
  onSave?: () => void;
  onIgnore?: () => void;
...
const ModalCheckConfirm: FC<IModalCheckConfirmProps> = props => {

  const { open, onClose, onIgnore, currentState } = props;
  const isApprovalState = currentState === "APPROVER_HEADQUARTER"
  ...
  return (
    ...
    >
      <Box>
        ...
        >
          { isApprovalState ? "DANH MỤC HỒ SƠ CẦN PHÊ DUYỆT" : "DANH MỤC HỒ SƠ CẦN KIỂM SOáT" }
        </Typography>
        <IconButton
          onClick={onClose}
          color="error"
        ...
        <Box className="flex-center" >
          <Grid container spacing={3} >
            ...
            <Grid item xs={4}>
              <span style={{
                fontSize:"16px"
              }}>{isApprovalState ? "Ngày phê duyệt :" : "Ngày kiểm soát :"}<span style={{
                color: "#353535",
                fontWeight: 500
        ...
        <Box sx={SxModalComfirm} >
          <span className='text-16 corrdinator-status-chile'><span className='corrdinator-status-chile-success'>
                ...
                fontSize:"16px"
              }} className="text-primary font-medium">{isApprovalState ? "Tổng số trang đã phê duyệt:" : "Tổng số trang đã kiểm soát:"} </span>
            <span style={{
        ...
        <Box>
          <TableCheck data={getDataPassPosition?.passed_positions ?? []} isApprovalState={isApprovalState} />
        </Box>
      </Box>
    </Modal>
  )
}
export default ModalCheckConfirm;