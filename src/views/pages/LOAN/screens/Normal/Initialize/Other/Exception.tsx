...
import { addNewException, deleteAllException, saveLOANNormalOther, updateStorageOtherReport } from 'features/loan/normal/storage/other/action';
import { getExeptions, getReport } from 'features/loan/normal/storage/other/selectors';
...
const OtherException: FC = () => {
  ...
  const onConfirmDelete = () => {
    showBackdrop(true)
    dispatch(deleteAllException(''));
    dispatch(saveLOANNormalOther(0, 'delete'));
  ...
  const handleChangeReport = (value : string) => {
    dispatch(updateStorageOtherReport(value))
  }
// console.log(Exceptions.every(item => item.detailList.length >= 1),'Exceptions.every(item => item.detailList.length >= 1)')
  return <Box className="mt-6">
    ...
    <ModalConfirm
      open={isModalConfirm}
      children="Bạn có chắc chắn muốn xóa tất cả"
