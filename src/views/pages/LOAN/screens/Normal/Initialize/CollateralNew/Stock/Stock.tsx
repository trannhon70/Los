...
const Stock: FC<StockProps> = (props) => {
  ...
  const [deleteStock, setDeleteStock] = useState<ISubItems | null>(null);
  ...
  const onHandleConfirmCer = () => {
    if(optionsData.length === 1 ){
      ...
      const dataPos = deleteStock?.activeUUID /// active local
      if (deleteStock?.price_cert_asset_uuid) {
        dispatch(deleteCollateralItem(
          dataPos ?? "",
          {
            price_cert_uuid: collateralData.price_cert_uuid ?? "",
            price_cert_asset_uuid: deleteStock?.price_cert_asset_uuid ?? "",
  ...
  const onHandleCancelConfirmStock = () => setDeleteStock(null);
  const onHandleClickMenuStock = (menu: IGroupListBase, position: number) => {
    let dataMenuCer = SubTypeItems?.find((cer, index) => index === position);
    setDeleteStock(dataMenuCer ?? null);
  };
  ...
  return (
    <Fragment>
      ...
      <Table>
        <TableBody>
          <TableRow>
            ...
            <TableCell width={'20%'}>
              <Typography sx={SxCollatertalCommon}>Chứng khoán</Typography>
            </TableCell>
            <TableCell>
            </TableCell>
                ...
                <TableRow>
                  <TableCell width='100%' sx={{
                    ...
                    <GroupListBase 
                      ...
                      onDelete={onHandleClickMenuStock}
                      options={optionsData}
                  ...
                  <TableCell width='100%' sx={{ border: 'none' }}>
                    ...
                    <Grid container spacing={3}>
                      ...
                      <Grid item xl={3}>
                        <Input label="2. Giá trị chứng khoán (VNĐ)"
                          placeholder="Nhập giá trị chứng khoán (VNĐ)"
                          ...
                          format
                          message={
                            getMessage('value', { position: dataItems?.activeUUID ?? '' }) && `${getMessage('value', { position: dataItems?.activeUUID ?? '' })} chứng khoán`
                          }
                          disabled={ ruleDisabled }
                        />
                      </Grid>
                      ...
                      <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
                        ...
                        >
                          ...
                          <TabPanel padding={false} value={CurrentTab} index={1}>
                            <TabPanel padding={false} value={CurrentTab} index={1}>
                            <ButtonDetailSubItemAttachment dataItems={dataItems} masterData={{ uuid: collateralData?.uuidActiveData ?? "", uuidActive: subType?.uuidActiveSubtype ?? "", collaretalType:'STOC' }}  />
                              <CardInside title="I. Thông tin chứng khoán" fieldsetClass="px-4" classBody="h-full p-6" >
                                <Grid container spacing={3}>
                                  ...
                                  <Grid item xl={3} lg={12} md={12} sm={12} xs={12}>
                                    <Input
                                      ...
                                      required
                                      value={(dataItems?.typeCollateral === 'RPRO' ? dataItems.status_flag?.id : dataItems?.status)}
                                      onDebounce={(val) => {
      ...
      <ModalConfirm
        ...
        onClose={onHandleCancelConfirmStock}
        onConfirm={onHandleConfirmCer}
      >
        <Box className="text-18 font-medium text-primary text-center">
          Bạn có chắc chắn muốn xóa chứng khoán
        </Box>
      </ModalConfirm>
    </Fragment>
  )
