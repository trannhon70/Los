...
const InformationTypeLand: FunctionComponent<CTXDInformationTypeLandProps> = (props) => {
  ...
  const curentItem = useSelector(getLOANormalStoreDataItemActive(uuIdData, activeSubType));
  ...
  const onHandleClickMenuCTXDTypeLand = (menu: ObjectListMenuItem, position: number) => {
    ...
    if(optionsDataCTXDLandType.length === 1){
      notify(" TSĐB phải ít nhất có 1 loại CTXD ","warning");
  ...
  const onHandleConfirmCTXDLandtype = () => {
    ...
    if (!_uuidDelete) {
      ...
      console.log(_uuidDelete,'_uuidDelete')
      dispatch(removeLandCTXDType(_uuidDelete, {
        ...
        land_const_item_uuid: data?.dataCTXDLand.find(i => i.activeUUIDCTXDLand === data.activeCTXDLand)?.land_const_item_uuid ?? "",
        land_const_item_detail_uuid: deleteIdLandType?.land_const_item_detail_uuid ?? "",
      }));
      ...
      <Grid container spacing={3} className='mt-5'>
        ...
        <Grid item xl={3} lg={12} md={12} sm={12} xs={12}>
          <Input
            ...
            value={dataCTXDLandTypeDetails?.certificate_floors?.toString() ?? ""}
            disabled={isDisiableInput || ruleDisabled}
            // type="number"
            // format
          />
        ...
        <Grid item xl={3} lg={12} md={12} sm={12} xs={12}>
          <Input
            ...
            value={dataCTXDLandTypeDetails?.floors?.toString() ?? ""}
            disabled={isDisiableInput || ruleDisabled}
            // type="number"
            // format
          />
