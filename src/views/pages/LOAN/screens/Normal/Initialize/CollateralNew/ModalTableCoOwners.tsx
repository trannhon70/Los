...
import { getInfoAllUserLegal, getLoanNormalSubTypeItemsActive } from "features/loan/normal/storage/collateralV2/selector";
...
import { setCollaretalCertificate, setCollaretalOwnerModal, setCollaretalOwnerType } from "features/loan/normal/storage/collateralV2/actions";
...
const ModalTableCoOwners: FunctionComponent<IModalTableCoOwnersProps> = (props) => {
  ...
  const [separate, setSeparate] = useState<string | null>(null)
  ...
  const onHandleConfirm = () => {
    ...
    dispatch(setCollaretalOwnerModal('',{
      uuidActiveData: uuIdData ?? '', 
      uuidActiveSubtype: activeSubType ?? '',
      objdataLegal: temp?.filter( i => i.person_uuid === separate)
    }))
            ...
            >
              ...
              
              {
                ...
                (()=> {
                  switch (type) {
                    case 'CO_BORROWER':
                       infoLegal = DataCoOwners?.filter( i => i.type !== 'related')
                       break;
                    case 'THIRD_PARTY':
                      infoLegal = DataCoOwners?.filter( i =>  { return i.type !== 'borrower' && i.type !== 'marriage' && i.type !== 'related' })
                      break;
