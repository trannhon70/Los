...
const Goods: FC<MachineProps> = (props) => {
  ...
  const onHandleConfirmCer = () => {
    if(optionsData.length === 1 ){
      ...
      const dataPos = deleteGoods?.activeUUID /// active local
      if (deleteGoods?.price_cert_asset_uuid) {
        dispatch(deleteCollateralItem(
          dataPos ?? "",
          {
            price_cert_uuid: collateralData.price_cert_uuid ?? "", 
            price_cert_asset_uuid: deleteGoods?.price_cert_asset_uuid ?? "",
              ...
              <Collapse unmountOnExit={true} timeout={1000} in={isOpen} sx={{
                ...
                <TableRow>
                  <TableCell width='100%' 
                    ...
                    <GroupListBase labelAdd='Thêm vật tư hàng hóa'
                      onAdd={onAdd}
                    ...
                    <Grid container spacing={3}>
                      ...
                      <Grid item xl={3}>
                        <Input label="2. Giá trị hàng hóa (VNĐ)"
                          placeholder="Nhập giá trị hàng hóa (VNĐ)"
                          ...
                          format
                          message={
                            getMessage('value', { position: dataItems?.activeUUID ?? '' }) && `${getMessage('value', { position: dataItems?.activeUUID ?? '' })} hàng hóa`
                          }
                          disabled={ ruleDisabled }
                        />
                      </Grid>
                      ...
                      <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
                        ...
                        >
                          ...
                          <TabPanel padding={false} value={CurrentTab} index={1}>
                            <ButtonDetailSubItemAttachment dataItems={dataItems} masterData={{ uuid: collateralData?.uuidActiveData ?? "", uuidActive: subType?.uuidActiveSubtype ?? "",collaretalType:'GODS' }}  />
                            <CardInside title="I. Thông tin hàng hóa" fieldsetClass="px-4" classBody="h-full p-6" >
                              <Grid container spacing={3}>
