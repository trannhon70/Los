...
const DepartmentInfo: FC<DepartmentInfoProps> = (props) => {
  ...
  const onHandleConfirmDeInfo = () =>{
    ...
    else if(optionsDep.length <= 1){
      notify('Không thể xóa, phải có ít nhất 1 CHCC', 'error');
    }else{
      dispatch(
        removeCollaretalDepartmentInfo(uuidDelete, {
          ...
          apart_room_uuid: deleteIdDeInfo?.apart_room_uuid ?? "",
        })
      );
        ...
        <Grid container spacing={3} className="pl-4 pb-4">
          ...
          <Grid item xl={3} lg={3} md={3} sm={12} xs={12}>
            <Input
              ...
              required
              maxlength={100}
              message={ getMessage('certificate_area_apa_info', {position: itemActive?.activeUUID ?? '', active: data?.departmentInfoActiveUUID ?? ''})}
              />
          </Grid>
          <Grid item xl={3} lg={3} md={3} sm={12} xs={12}>
            <Input
