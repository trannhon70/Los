...
import { saveLOANNormalStorageLOANModalAttachFile } from "features/loan/normal/storage/loan/actions";
import { EmptyFileLoan } from "features/loan/normal/storage/loan/emptyData";
import {
  getDataConfigDocument, getLOANNormalStorageListDocumentInfos
...
const AttachmentModalServices = ({
  ...
  const dataFilelist = useSelector(getLOANNormalStorageListDocumentInfos);
  ...
  const onSaveFileLoan = () => {
    if (localData && localData?.length > 0) {
      ...
      dispatch(saveLOANNormalStorageLOANModalAttachFile(dataUpload));
    }
  };
            ...
            document_group : parentDoc.document_group.map(doc => {
              if(doc_uuid.includes(PREFIX_LOCAL)){
                if(doc.uuid === doc_uuid){
                  return {
                    ...doc,
                    child_files: doc.child_files ? [...doc.child_files, EmptyFileLoan()] : [EmptyFileLoan()]
                  }
                }
                else return {...doc}
              }
    ...
    remove: {
      ...
      doc: ({ uuids, action_uuid }) => {
        if (uuids.length < 3) {
          ...
          // dispatch(
          //   LOANActions.removeLOANNormalStorageLOANParentDoc(parentDoc_uuid)
          // );
        } else {
          // DOC
          const [AllPre = "", parentDoc_uuid = "", doc_uuid = ""] = uuids;
          ...
          // dispatch(
          //   LOANActions.removeLOANNormalStorageLOANDoc(doc_uuid, {
          //     parentDoc_uuid,
          //   })
          // );
        }
      ...
      file: ({ uuids, action_uuid }) => {
        ...
        // dispatch(
        //   LOANActions.removeLOANNormalStorageLOANFile(file_uuid, {
        //     parentDoc_uuid,
        //     doc_uuid,
        //   })
        // );
    ...
    update: {
      doc: ({ uuids, action_uuid }, value, option) => {
        if (uuids.length < 3) {
          ...
          // dispatch(
          //   LOANActions.setDataLOANNormalStorageLOANParentDoc(value, {
          //     parentDoc_uuid,
          //     currentData: {
          //       label: option?.label ?? "",
          //       value: option?.value ?? "",
      ...
      fileDesc: ({ uuids, action_uuid }, value) => {
        ...
        // dispatch(
        //   LOANActions.setDescriptionLOANNormalStorageLOANFile(value ?? "", {
        //     parentDoc_uuid,
        //     doc_uuid,
        //     file_uuid,
        //   })
    ...
    chooseFile: ({ uuids, action_uuid }, dataFiles) => {
      ...
      dataFiles.forEach((file) => {
        if(file.size && file.size > size/1024){
          ...
          // dispatch(
          //   LOANActions.setDataLOANNormalStorageLOANFile(result, {
          //     parentDoc_uuid,
          //     doc_uuid,
          //     file_uuid,
          //   })
