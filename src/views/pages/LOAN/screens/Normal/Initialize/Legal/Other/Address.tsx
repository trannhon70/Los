...
export interface IOtherAddressRef {
  validate(): boolean;
}

export interface IOtherAddressProps{}


type TOtherAddressComponent = ForwardRefRenderFunction<
  IOtherAddressRef, 
  IOtherAddressProps
>


const OtherAddress: TOtherAddressComponent = (props, ref) => {
  ...
  const onClose = () => {
    setModalAdd(null);
    const addresUp = address.map((item, i) => {
      if(!item.primaryFlag){
        return {...item}
      }
...
export default forwardRef(OtherAddress);