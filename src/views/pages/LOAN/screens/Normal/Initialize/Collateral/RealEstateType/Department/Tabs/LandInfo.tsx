...
const LandInfo: FC<LandInfoProps> = (props) => {
  ...
  useEffect(() => {
     ...
     */
    let isCheck = dataDetails ? ( dataDetails.activeUUIDCertificateUsePurposes?.length > 0 ? false : true ) : true;
...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const onHandleConfirmCer = () =>{
    ...
    if (!uuidDelete){
      ...
      dispatch(
        removeLandCertificateInfoUsePurposes(uuidDelete, {
          uuidData: uuIdData,
          uuidSubType: uuIdSubType,
          uuidItem: SubTypeItemsActive,
        })
      );
      notify('Xóa mục đích thành công', 'success');
    }
        ...
        <Grid container spacing={3} className="pl-4 pb-4">
          <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
            <SelectLocation
              ...
              before={
                ...
                >
                  <Input
                    ...
                    message={ getMessage('address', {position: itemActive?.activeUUID ?? ''})}
              ...
              disabled={ruleDisabled}
              message={[
                getMessage('province', {position: itemActive?.activeUUID ?? ''}),
                getMessage('district', {position: itemActive?.activeUUID ?? ''}),
                getMessage('ward', {position: itemActive?.activeUUID ?? ''})
              ]}
